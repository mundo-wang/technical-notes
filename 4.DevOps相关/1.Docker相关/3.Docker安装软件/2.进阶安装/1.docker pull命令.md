...
docker pull [OPTIONS] <REPOSITORY>@<DIGEST>
```

- `REPOSITORY`：指定镜像名称，通常是`<仓库>/<镜像名>`的形式，如果没有指定仓库，默认从`DockerHub`中拉取。
...
这些`tar`文件存储在云服务器的`/opt/images`目录下。由于这些文件占用较多硬盘空间，已将它们转移到个人`U`盘中。
