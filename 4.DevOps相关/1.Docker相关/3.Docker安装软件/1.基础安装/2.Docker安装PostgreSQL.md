...
`PostgreSQL`的默认用户名名称为`postgres`，且`PostgreSQL`有一个默认的初始数据库也叫`postgres`。我们可以使用`Navicat`连接上`PostgreSQL`，密码为上面设置的`123456`。

`PostgreSQL`创建一个数据库挺费劲，跟着下图填即可：
