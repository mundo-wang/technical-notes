...
复制完文件后我们需要修改时区配置。编辑`conf.d`目录下的`my.cnf`文件，在`[mysqld]`下面添加如下配置：
...
如果没有`my.cnf`文件，我们可以在`conf.d`目录下新建`mysql-timezone.cnf`文件，内容如下：
...
如果需要修改密码，不可以停止并删除上面的容器后更改`-e`的`MYSQL_ROOT_PASSWORD`参数后重新启动，因为这可能会引发一些问题。建议使用以下命令进行密码重置，这样可以在不停止`MySQL`的情况下动态更新密码：
