...
创建并编辑`redis.conf`文件，这里需要给`<password>`填充为自己的密码：
...
其中，`appendonly yes`表示开启`Redis`的持久化，`dir /data`为容器内部路径，作用是设置数据持久化文件存储路径。
...
其中，`redis-server /usr/local/etc/redis/redis.conf`为容器内部路径，表示启动`Redis`容器时加载对应的配置文件。
...
在执行命令时，系统要求输入密码，我们输入`auth <password>`以进行身份验证：
...
修改后，再试试使用`Redis Desktop Manager`（`RDM`）能不能连上。
