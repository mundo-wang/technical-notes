...
执行上面的文件需要`docker compose`命令，我们这里使用国内镜像源下载它：
...
> **注意**：有一些`Compose`的版本，查看版本的命令是`docker-compose --version`，执行命令也要使用`docker-compose`，而我们下载的版本的执行命令是`docker compose`，下同。
...
再次使用`docker ps -a`查看容器时，可以查看到容器为退出状态：
...
同理，如果文件名不叫`docker-compose.yml`，这两条命令也可以通过`-f`选项来指定文件名。