...
如果希望容器开机自启动，可以使用`--restart always`选项。这样可以让容器在宿主机启动时自动启动，非常适合在本地虚拟机中运行容器的场景。真实生产环境中，一般会使用`on-failure`或`unless-stopped`等策略。

这几种重启策略在以下场景下的表现如下所示：

| 场景                     | `always` | `on-failure` | `unless-stopped` |
| ------------------------ | -------- | ------------ | ---------------- |
| 正常退出（退出码`0`）    | 重启     | 不重启       | 重启             |
| 异常退出（退出码非`0`）  | 重启     | 重启         | 重启             |
| 手动`stop`后             | 不重启   | 不重启       | 不重启           |
| 手动`stop`后`daemon`重启 | 重启     | 不重启       | 不重启           |
| 正常退出后`daemon`重启   | 重启     | 不重启       | 重启             |

宿主机重启本质上会导致`daemon`重启，所以两者触发逻辑相同，行为结果完全一致。