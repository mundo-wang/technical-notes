...
sudo yum makecache fast # 在CentOS9以上版本中，去掉fast或将其换为--timer
...
接下来我们需要配置`Docker`镜像加速器，原因是从`DockerHub`（https://hub.docker.com/）上下载镜像太慢。
...
> 如果希望在拉取镜像时优先使用企业私有仓库，而在未命中时再回退到`DockerHub`或其他镜像站点，则需要将企业私有仓库地址配置在镜像源列表的第一位。

配置完上面内容后，还需要设置`Docker`的开机自启动，命令如下：
