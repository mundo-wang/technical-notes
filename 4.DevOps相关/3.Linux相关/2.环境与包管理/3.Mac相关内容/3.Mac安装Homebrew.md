...
安装`Homebrew`时会自动触发`Xcode Command Line Tools`的安装，`Git`会随之一并进行安装。这一步耗时较长，耐心等待。

在`macOS`上，通过该脚本安装`Homebrew`后，`brew`可执行文件的位置取决于本机芯片架构：

1. `Apple Silicon`：`Homebrew`根目录为`/opt/homebrew/`，`brew`可执行文件位于`/opt/homebrew/bin/brew`。

2. `Intel`芯片：`Homebrew`根目录为`/usr/local/Homebrew/`，`brew`可执行文件位于`/usr/local/bin/brew`，它是指向`/usr/local/Homebrew/bin/brew`的软链接。
...
需要注意的是，这个`Git`是`Apple`维护的定制版本，版本号通常落后于`Git`官方最新版。

如果对版本有要求，可以通过`Homebrew`单独安装官方版本：

```sh
brew install git
```

安装后`Homebrew`版本的`Git`会优先于系统自带版本被使用。

如果你的本地网络能访问`GitHub`，推荐使用下面命令恢复官方源：

```sh
git -C "/opt/homebrew" remote set-url origin https://github.com/Homebrew/brew
```

安装完`Homebrew`后，系统会提示其配置文件被存放在`~/.zprofile`中。但该文件在重启电脑后并不会被自动`source`，这意味着每次重启都需要手动执行一次`source ~/.zprofile`。为了解决这个问题，我们可以编辑`~/.zshrc`文件，将`source ~/.zprofile`的命令添加进去，这样每次打开终端时都会自动加载`Homebrew`的配置。

`Homebrew`安装完成后会提示将初始化脚本写入`~/.zprofile`，如下所示：

<img src="image/image-20260604145413210.png" alt="image-20260604145413210" style="zoom:40%;" />
