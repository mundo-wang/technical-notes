...
这里建议设置字体为`Courier New`，字号大小为`18`，字符间距为`0.9`，行间距为`1.05`。
...
`Normal Colors`默认的白色偏灰，显示在屏幕上不清晰，这里建议将其调整到最白：

<img src="image/image-20251210145640609.png" alt="image-20251210145640609" style="zoom:40%;" />
...
这里也可以将凭证名设置为具体的密码值，这样便于直接识别当前密码内容，同时也支持一个密码复用连接多台机器，但需要注意安全性，应避免在多人可访问或不受控的环境中使用。
