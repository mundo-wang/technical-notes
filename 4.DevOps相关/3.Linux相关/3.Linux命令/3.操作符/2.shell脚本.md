`Shell`脚本是由一系列`Shell`命令组成的文本文件，通常用于自动化执行系统操作。脚本文件一般以`.sh`为后缀，并由`Shell`解释器逐行解析执行。`Shell`脚本有两种常见的执行方式：使用`sh`命令或使用`./`方式。二者的主要区别在于是否遵循脚本开头的`shebang`（即解释器声明，例如`#!/bin/bash`或`#!/bin/zsh`）：
