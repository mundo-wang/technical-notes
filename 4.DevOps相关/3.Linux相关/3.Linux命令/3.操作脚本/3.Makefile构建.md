...
使用`make <target>`可以执行指定目标下的`command`。如果直接运行`make`而不指定目标，会默认执行文件中的第一个目标。

通常需要在`Makefile`文件所在的目录中执行`make`命令。由于`Makefile`一般位于项目根目录，因此`make`命令通常在项目根目录下执行。`make`在运行时会默认查找名为`Makefile`或`makefile`的文件，因此项目中的构建文件通常采用这这两个名字之一。
