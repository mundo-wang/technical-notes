...
```sh
kill -9 1234567
kill -SIGKILL 1234567
```

二者是等价的，我们通常使用前者，也就是数字表示。

通过`kill`命令发送信号终止进程时，属于异常退出，其退出码为非`0`值。退出码的计算规则为`128` + 信号编号，例如：

- 被`SIGKILL (9)`终止：退出码为`128 + 9 = 137`。
- 被`SIGTERM (15)`终止：退出码为`128 + 15 = 143`。

在`Shell`中，`$?`表示上一个命令的退出码，`$!`表示最近一个后台进程的`PID`。可以使用`kill -9 $!`向最近一个后台进程发送终止信号，强制立即终止该进程；也可以通过`echo $?`查看上一个命令的退出码。
