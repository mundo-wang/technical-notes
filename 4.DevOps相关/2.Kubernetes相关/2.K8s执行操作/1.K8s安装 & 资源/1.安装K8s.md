...
对于`Mac`环境的`CentOS Stream release 9`版本，配置`yum`仓库的命令如下：
...
对于`Mac`环境的`CentOS Stream release 9`版本，安装`K8s`相关组件命令如下：
...
对于`Mac`环境的`CentOS Stream release 9`版本，初始化集群的命令如下：
...
这表明`kubelet`已成功连接到`containerd`，但`containerd`的`CRI`接口不可用，同时内核网络模块因为缺少`bridge-nf-call-iptables`导致未正确加载。
