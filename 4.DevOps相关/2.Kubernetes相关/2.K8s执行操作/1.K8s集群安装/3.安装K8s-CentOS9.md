...
一主二从的`K8s`集群搭建完毕后，若过段时间发现某台`node`节点状态变为`NotReady`，可参考以下排查与修复流程。

一、问题现象

在`master`节点执行`kubectl get nodes`，发现`node2`节点状态为`NotReady`：
...
在`master`节点执行`kubectl describe node node2`，在`Conditions`部分可以看到如下信息：
...
登录到`node2`，执行`systemctl status kubelet`，发现`kubelet`进程本身是运行中的。执行下面命令查看日志：
...
`Static hostname`未设置，系统开机初期短暂使用默认值`localhost.localdomain`。由于`kubelet`配置了开机自启，它在`systemd`完成主机名初始化之前就已启动，读取到了错误的主机名并完成初始化，后续一直沿用`localhost.localdomain`向`API Server`上报，从而引发权限错误，节点持续`NotReady`。
