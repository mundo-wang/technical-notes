...
>
> 注：自`Kubernetes 1.20`版本起，控制节点的官方名称已由`Master`更名为`Control Plane`，但本文为便于理解与讲解，仍沿用`Master`这一称呼。
...
一主二从的`K8s`集群搭建完毕后，若过段时间执行`kubectl get nodes`发现某台`node`节点状态变为`NotReady`：
