...
执行`kubectl apply`时，`Kubernetes`会在资源的`metadata.annotations`中写入一个名为`kubectl.kubernetes.io/last-applied-configuration`的注解，记录本次`apply`时传入的完整配置内容。下次再执行`apply`时，`Kubernetes`会以这份注解作为上一次的配置基线，与当前`YAML`文件进行三方对比，三方分别是上次应用的配置、当前集群中资源的实际状态、本次传入的新配置，在此基础上计算出差异并执行最小化变更。

这一机制带来了两个重要特性。其一是幂等性，同一份`YAML`无论执行多少次`apply`，只要内容不变，集群状态就不会改变，不会像`create`那样因资源已存在而报错。其二是增量更新，`apply`只变更有差异的字段，未出现在`YAML`中但已存在于集群的字段（例如通过命令行手动添加的注解）默认会被保留。

需要特别留意的是，若某个字段曾通过命令行手动修改过，但该字段同时也出现在`YAML`文件中，再次`apply`时`YAML`中的值会覆盖手动修改的值。混用命令行与`apply`的场景下，以`YAML`文件为最终权威是更安全的做法。
...
现在将`nginx-app.yaml`中`Deployment`的镜像修改为`nginx:1.25`，副本数调整为`3`，再次执行`apply`：
...
`--dry-run=server`会将请求发送至`API Server`做完整校验，包括字段合法性与权限检查，但不持久化到`etcd`，比`--dry-run=client`的校验更严格，适合在正式变更前做确认。
