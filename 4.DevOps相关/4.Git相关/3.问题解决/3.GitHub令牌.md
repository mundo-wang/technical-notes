...
如果`telnet`连接失败，说明网络可能限制了本机对`443`端口的访问。此时可以尝试切换网络，关闭或开启`VPN`，必要时也可以配置代理来绕过网络限制：
...
> `2025-4-14 12:58:07`更新：目前开启`Clash`的`TUN`模式后，反而无法连接到`443`端口；而关闭`TUN`模式后，连接恢复正常。
