...
对于`SSH`的连接方式，上面配置的`HTTP`与`HTTPS`代理不再适用，我们在`~/.ssh/config`文件中添加以下内容：
...
上述内容仅适用于`Mac`环境；`Windows`环境可先安装`Nmap`，并将`ncat.exe`加入`PATH`环境变量，配置内容如下：
...
而`ssh -T git@github.com`命令可以正常输出信息，可以先尝试重启`Clash Verge`软件，再重新`git push`。

即使未在`config`文件中手动配置代理，`SSH`连接方式通常也不会受到影响，因为`SSH`本身已经内置并配置了代理机制。
...
<img src="image/image-20260120135607855.png" alt="image-20260120135607855" style="zoom:40%;" />
