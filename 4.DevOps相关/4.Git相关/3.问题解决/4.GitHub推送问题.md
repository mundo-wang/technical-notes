...
对于`SSH`的连接方式，上面配置的`HTTP`与`HTTPS`代理不再适用，而应该在`~/.ssh/config`文件中添加以下内容：
...
```
Hi mundo-wang! You've successfully authenticated, but GitHub does not provide shell access.
```
