...
如果遇到`SSH`连不上的问题，可以在`~/.ssh/config`文件中配置代理，如下所示：
...
这要求本机有科学上网工具，例如使用`Clash Verge`默认监听`7897`端口，或者`XQNetwork`监听`7890`端口。

使用以下命令，可以查看`SSH`与`GitHub`建立连接的详细过程，便于排查问题：
