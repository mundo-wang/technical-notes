...
> **注意：**在执行该命令之前，`feature`分支上的所有提交应仅保留在本地，尚未推送到远程仓库。
...
4. 将之前移除的本地提交逐个应用到新的`HEAD`上，使本地改动基于最新的`release`分支代码。

当前的`feature`分支是基于旧版本的`release`分支创建的。在上面命令中，需要将其更新到`release`分支的最新提交。执行该操作后，`feature`分支将基于最新的`release`分支，同时保留自身的本地提交。

如果在`rebase`过程中遇到冲突，需要手动解决冲突，然后执行下方命令：
...
此分支一旦合并到主分支，就作废了。我们可以删除掉它的本地分支和远程分支：
