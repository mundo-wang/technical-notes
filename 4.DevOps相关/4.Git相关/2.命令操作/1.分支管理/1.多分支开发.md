...
此外，`rebase`命令原则上只应执行一次。执行完成后，应尽快将该分支合并到`release`分支并予以废弃，不应再继续基于该分支进行开发。如果确实需要在该分支上继续开发，并在开发后再次执行了`rebase`操作，常规推送会被拒绝。在能够明确保证该分支仅由个人使用、不涉及他人协作的前提下，同样可以通过上述命令进行处理。
...
该合并与推送步骤通常由具有更高权限的人员（如项目负责人）来执行，以确保`release`分支的稳定性与纯洁性。
