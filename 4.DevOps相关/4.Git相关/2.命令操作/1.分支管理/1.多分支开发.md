...
在执行`rebase`命令之前，不要将`feature`分支的提交推送到远程仓库。因为`rebase`会重写当前分支相对于`release`分支的本地提交`hash`值，如果这些提交已经被推送到远程，再次推送时会导致提交冲突，需要通过下面命令解决：
...
此时我再将`feature`分支`merge`到`release`分支，就会导致该次合并无法以`fast-forward`方式完成：
...
在该`merge`过程中一旦出现代码冲突，就需要在`release`分支上进行解决，处理成本较高。
