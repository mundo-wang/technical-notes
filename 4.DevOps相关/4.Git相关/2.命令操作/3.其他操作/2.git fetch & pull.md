`fetch`的中文释义为「取来」。在本地执行`git fetch`命令时，`Git`会从远程仓库获取最新分支状态，并更新本地的远程跟踪分支信息。通过该命令，我们可以在本地查看远程仓库的最新状态，同时不会影响当前的本地工作分支。`git fetch`不会自动将远程分支的更新合并到本地分支中。如果需要将远程跟踪分支的最新内容合并到本地分支，则需要手动执行合并操作。
...
`git pull`与`git fetch`的主要区别在于，`git pull`是一个复合操作。默认情况下，执行`git pull`时：

1. 首先执行`git fetch`，从远程仓库获取最新分支状态，并更新对应的本地远程跟踪分支。
2. 然后执行`git merge origin/<branch-name>`，将当前检出分支的远程跟踪分支合并到本地分支。

如果希望明确指定远程仓库和分支，使拉取操作仅针对指定的远程分支，可以执行：
