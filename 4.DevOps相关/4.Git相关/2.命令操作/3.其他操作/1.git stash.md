在临时切换分支或中断工作时，可以使用`git stash`来临时存储当前工作目录和暂存区中所有尚未`commit`的修改。
...
#### 2. 临时存储修改但未`commit`的内容，并附带`message`说明
...
默认情况下，`git stash apply`命令会将`stash`中的所有修改恢复到工作区，包括保存前位于暂存区的修改，因此原本的暂存区状态不会恢复；如需同时恢复暂存区状态，可使用`git stash apply --index`。
...
但如果冲突发生在`stash`内容与已`commit`的代码之间，`Git`的处理方式与常规冲突相同，会保留冲突标记，允许手动解决冲突后继续操作。若不想处理冲突、希望回退此次`stash`操作，可执行以下命令：

```
git reset --hard HEAD
```

`stash`记录本身不受影响，后续随时可以重新应用该暂存记录。
...
#### 7. 恢复最后一次`stash`并从`list`中删除

```bash
git stash pop
```

该命令相当于先后执行了`git stash apply stash@{0}`和`git stash drop stash@{0}`的组合操作。

默认情况下，`git stash pop`命令会将`stash`中的所有修改恢复到工作区，包括保存前位于暂存区的修改，因此原本的暂存区状态不会恢复；如需同时恢复暂存区状态，可使用`git stash pop --index`。
