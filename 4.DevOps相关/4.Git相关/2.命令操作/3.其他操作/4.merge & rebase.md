...
> **注意：**在执行`merge`操作后，提交`C`、`D`、`E`的哈希值不会发生变化，使用`git log`命令查看提交记录时，会按照各自的提交时间顺序进行排序。若未产生冲突，提交`M`是`merge`操作自动生成的合并提交，起到标记两条分支汇合点的作用；若发生冲突，在解决冲突后需要执行`git add`和`git commit`，此时创建的提交`M`将包含人工处理后的最终代码快照。
