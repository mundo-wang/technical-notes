...
- `git push --force-with-lease`的行为是：在执行强制推送前，会校验远程分支当前指向的提交，是否与本地上一次记录的远程分支提交状态一致。如果二者不一致，说明远程分支产生了新的提交，此时推送会被拒绝，从而避免误覆盖他人已提交的内容。

除非明确确认分支只由你个人使用，否则应选择使用`--force-with-lease`而不是`--force`。
