...
要查看提交之间的关系（如`merge`操作的分岔），使用以下命令：
...
```sh
*   c5f5678 Merge branch 'feature'     # master分支上的merge提交
|\
| * 4a6d7b9 Add new feature            # feature分支上的提交
* | 3b2f1a6 Update README              # master分支上的提交
|/
...
我们也可以使用`HEAD`代表当前分支最新提交，适合在已切换到目标分支时快速操作：

```sh
git checkout feature
git log --oneline HEAD..release
```

还可以使用`--left-right --graph`，支持一次性双向对比，无需执行两次命令：

```
git log --oneline --left-right --graph release...feature
```

输出中，`<`开头的提交属于`release`独有，`>`开头的属于`feature`独有。注意`...`（`3`个点）是对称差集语法，与单向对比的`..`（`2`个点）含义不同，`..`只看一侧的差异，`...`同时呈现双侧。

在`JetBrains`的`IDE`中，可以在`release`分支的`Git`操作中选择`feature`分支并点击`Compare with 'release'`：
...
- 本地引用`ref`：每个分支对应一个，用于指向该分支当前被定义为分支顶端的提交。在非重写历史的情况下，执行`commit`操作后，该分支对应的`ref`会推进到新提交。所谓重写历史，指的是在不产生新提交的前提下，修改已有提交的内容或顺序，导致原提交的`SHA-1`发生变化或被丢弃，常见操作包括`rebase`、`reset`等。
...
`HEAD^`等价于`HEAD^1`，表示`HEAD`的第一个父提交；`HEAD^2`表示`HEAD`的第二个父提交。普通提交只有一个父提交，而合并提交通常有多个父提交，`HEAD^1`表示当前分支在合并前的最新提交，`HEAD^2`表示被合并进来的分支对应的最新提交。
...
这时当前`HEAD`指向提交`M`，`HEAD^1`指向提交`C`，`HEAD^2`指向提交`E`。
...
> 该命令不仅可以对`HEAD`使用，也可以对完整或缩写的提交`hash`使用。
