授权的本质可以归结为回答三个核心问题，即谁能够对什么资源执行什么操作。

主体是权限的发起者，表示「谁」拥有权限，通常包括用户、用户组、角色、服务账号等，例如用户`Alice`。对象是权限作用的目标，表示「对什么资源」进行访问，可以是某个接口、某张表、某个字段、某个文件，甚至是一个微服务。动作则定义主体能够对资源执行的具体行为，表示「能做什么操作」，常见的操作包括读取、写入、更新、删除、执行等。
...
上面是一个基于角色的访问控制（`RBAC`）配置，我们可通过修改该文件将其调整为基于属性的访问控制（`ABAC`）等形式。
...
策略规则用于控制访问权限，涵盖接口、前端菜单与路由以及按钮等资源。

- 接口访问权限示例：`p, admin, /api/user, GET`，表示`admin`角色可访问`/api/user`接口并执行`GET`请求。

- 前端菜单或路由访问权限示例：`p, admin, menu:user_manage, view`，表示`admin`角色可访问唯一标识为`menu:user_manage`的菜单或路由，并具有查看权限。

- 按钮或操作级权限示例：`p, admin, button:user:add, click`，表示`admin`角色可对唯一标识为`button:user:add`的按钮执行点击操作，从而完成新增用户等业务功能。

需要注意的是，策略规则中的后两个字段并非固定格式，而是由业务自行定义。资源既可以是接口路径，也可以是菜单、按钮、文件等任意业务对象；操作既可以是`GET`、`POST`等`HTTP`方法，也可以是`view`、`click`、`edit`、`delete`等业务动作。因此，策略规则本质上描述的是「哪个主体」对「哪个资源」拥有哪些操作权限，其具体含义取决于业务系统对资源和操作的定义。
...
在`JWT`中会保存用户的`ID`，当认证完成后，系统会根据这个`ID`获取用户的角色信息，从而确定其可访问的前端菜单和可操作的按钮等。同时，在受限接口对应的中间件中，还会根据用户的角色信息，判断其是否具备对应的操作权限。
