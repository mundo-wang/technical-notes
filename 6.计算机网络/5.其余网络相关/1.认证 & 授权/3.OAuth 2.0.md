...
需要说明的是，实际项目中的「第三方登录」并非仅依赖`OAuth 2.0`协议即可完成，而是在`OAuth 2.0`授权框架基础上，结合身份提供方提供的扩展能力共同实现。因此，本文除了介绍`OAuth 2.0`协议本身外，也会结合微信登录等实际场景说明第三方登录的完整实现流程，例如获取`openid`等第三方唯一标识、建立本地用户体系、维护账号绑定关系、签发业务系统自身的会话凭证（通常为`JWT`）等。这些内容属于第三方登录的工程实践，并非`OAuth 2.0`协议本身规定的内容。

其中，与标准化身份认证相关的能力，属于`OIDC`（`OpenID Connect`）协议的职责范畴。`OIDC`构建于`OAuth 2.0`授权框架之上，通过引入统一的身份层，为客户端提供标准化、可验证的身份认证结果，使客户端能够确认当前登录用户的身份及本次认证结果的真实性。关于`OIDC`的设计背景、核心概念及认证流程，将在另一篇笔记中详细介绍。
...
授权服务器通过校验上述内容，确认当前令牌申请确实来自已注册的合法客户端，而非攻击者在截获授权码后伪造的请求，从而有效防止授权码被盗用后的冒充行为。校验通过后，返回`AccessToken`、`RefreshToken`与用户的唯一标识，例如微信会返回`openid`。服务端将令牌持久化存储，以便后续代表用户调用第三方平台接口（如拉取用户头像、昵称等基础资料）时使用。
