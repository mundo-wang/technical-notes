...
`OAuth 2.0`解决的核心问题是资源拥有者如何在不暴露自身凭证的前提下，授权第三方客户端访问其在资源服务器上的特定资源，例如允许小红书读取我微信的头像与昵称。平台内部的角色归属、菜单可见性、接口可调用性等属于业务层面的权限控制，由资源服务器基于`RBAC`或其他权限模型自行完成，授权服务器并不参与具体业务权限的判定。
...
客户端首先向授权服务器发起授权请求，用户在授权服务器完成身份验证并同意授权后，授权服务器返回授权码给客户端。客户端再将授权码传递给服务端，服务端携带授权码、客户端标识`client_id`与客户端密钥`client_secret`向授权服务器申请令牌，授权服务器校验通过后返回`AccessToken`与`RefreshToken`，服务端再将`AccessToken`返回给客户端。

> 这里的`client_id`与`client_secret`是开发者在授权服务器注册应用时一次性分配的一对凭证：`client_id`用于标识客户端身份，可公开使用；`client_secret`是与之配对的私密凭证，只保存在服务端，用于向授权服务器证明该请求确实来自合法的客户端本身，而非他人冒用授权码发起的请求。

客户端携带`AccessToken`向服务端发起请求，服务端凭此向资源服务器请求受保护的数据与服务；资源服务器将令牌转发至授权服务器完成有效性校验，校验通过后返回对应资源。若令牌已过期，资源服务器将过期信息回传给服务端，服务端随即使用`RefreshToken`向授权服务器申请新的`AccessToken`，从而避免用户重新经历一次完整的授权流程。

如果授权服务器判定`RefreshToken`本身也已过期，将拒绝本次刷新请求并返回对应的错误信息。在这种情况下，用户必须重新完成授权流程，以重新获取有效的`AccessToken`和`RefreshToken`。
...
服务端获取`AccessToken`、`RefreshToken`及对应的过期时间后，可将上述信息与用户标识一并写入`user_token`表，用于后续的会话管理。后续请求中，若`AccessToken`已过期，服务端将查表检查对应的`RefreshToken`是否过期：

- 若`RefreshToken`未过期，服务端会凭此向授权服务器申请新的`AccessToken`，更新`user_token`表中的访问令牌及其过期时间，并将刷新后的`AccessToken`返回给客户端，供后续请求携带使用。
- 若`RefreshToken`已过期，说明用户会话已失效，服务端将删除`user_token`表中对应的记录，并跳转至授权服务器的登录页面，引导用户重新完成授权流程。授权服务器颁发新的令牌后，向`user_token`表写入新的令牌信息。

用户手动登出系统时，服务端应根据传入的`AccessToken`删除`user_token`表中对应的记录，并同步调用授权服务器接口，移除对应的令牌信息，确保授权状态被清除。用户后续需重新登录获取新的`AccessToken`和`RefreshToken`。
