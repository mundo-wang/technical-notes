...
相较而言，`OAuth 2.0`的`AccessToken`通常具有较短的有效期。令牌过期后，客户端需要通过`RefreshToken`向认证服务器申请新的`AccessToken`。当用户主动下线或服务端执行强制下线时，认证服务器会删除对应的`AccessToken`与`RefreshToken`，客户端同时清除本地存储的`AccessToken`。即使访问令牌被第三方截获，由于它已在认证服务器端失效，因此无法被继续使用。
...
`OAuth 2.0`中的授权并非指单个平台内部的角色或页面权限分配，而是指跨平台的访问授权，即用户是否允许某个客户端在指定范围内访问资源服务器上的资源。平台内部的角色归属、菜单可见性、接口可调用性等仍属于业务权限，由资源服务器基于自身的`RBAC`或其他模型控制。认证服务器只负责确认“你是谁”以及“令牌是否可信”，不参与具体业务权限判定。
