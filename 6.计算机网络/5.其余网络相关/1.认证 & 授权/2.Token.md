...
需要特别说明的是，`JWT`的`Payload`部分仅经过`Base64UrlEncode`编码，并非加密，任何拿到`JWT`字符串的人都可以直接对`Payload`部分进行解码，还原出其中的原始`JSON`数据。这意味着不应在`Payload`中存放密码、身份证号等敏感信息，否则等同于将这些信息直接暴露在网络传输与客户端本地存储中。`JWT`真正的安全保障来自`Signature`部分，它确保`Payload`内容不会被篡改，而不是确保其不会被读取，这一点是使用`JWT`时容易被忽视的安全误区。
...
`OAuth`这个名字来源于`Open Authorization`，其中`Auth`表示授权，`2.0`表示该授权协议的第二个主要版本。`OAuth2.0`是一种用于在不暴露用户凭证的前提下，实现第三方应用安全访问受保护资源的授权框架。

`OAuth 2.0`解决的核心问题是：资源拥有者如何在不暴露自身凭证的前提下，授权第三方客户端访问其在资源服务器上的特定资源，例如允许知乎读取微信的头像与昵称。平台内部的角色归属、菜单可见性、接口可调用性等，属于业务层面的权限控制，由资源服务器基于`RBAC`或其他权限模型自行完成。授权服务器的职责仅限于确认「你是谁」与「令牌是否合法可信」，并不参与具体业务权限的判定。
...
1. 资源服务器将平台侧的用户信息同步至授权服务器，用于后续的用户身份校验与令牌颁发。
2. 客户端向授权服务器发起授权请求，用户完成身份验证后，授权服务器返回授权码。
3. 客户端传递授权码到服务端，服务端向授权服务器申请访问令牌和刷新令牌，并返回访问令牌给客户端。
4. 客户端携带访问令牌发送请求到服务端，服务端向资源服务器请求访问受保护的数据与服务。
5. 如果服务端发现访问令牌过期，使用刷新令牌向授权服务器申请新的访问令牌。

在`OAuth 2.0`中，`AccessToken`通常具有较短的有效期。当`AccessToken`过期后，服务端需要携带`RefreshToken`向授权服务器申请新的`AccessToken`。如果授权服务器判定`RefreshToken`本身也已过期，将拒绝本次刷新请求并返回对应的错误信息。在这种情况下，用户必须重新完成授权流程，以重新获取有效的`AccessToken`和`RefreshToken`。
...
当用户首次通过客户端发起请求并完成`OAuth 2.0`授权流程后，服务端将获取`AccessToken`、`RefreshToken`及对应的过期时间。并将上述信息与用户标识一并写入`user_token`表，用于后续的会话管理。在后续请求中，服务访问资源服务器时需携带`AccessToken`，资源服务器再基于该`AccessToken`向授权服务器发起校验请求，以确认其有效性。

如果查询到`AccessToken`有效，资源服务器会根据`user_id`确定用户角色，并授予相应权限。

如果`AccessToken`已过期，服务端将检查对应的`RefreshToken`是否过期：

- 若`RefreshToken`未过期，服务端会凭刷新令牌向授权服务器申请新的`AccessToken`，更新`user_token`表中的访问令牌及其过期时间，并将刷新后的`AccessToken`返回给客户端，供后续请求携带使用。
- 若`RefreshToken`也已过期，说明用户会话已失效，服务端将删除`user_token`表中对应的记录，并跳转至授权服务器的登录页面，引导用户重新完成认证流程。授权服务器颁发新的令牌后，向`user_token`表写入新的令牌信息。

用户手动登出系统时，服务端应根据传入的`AccessToken`删除`user_token`表中对应的记录，并同步调用授权服务器接口，移除该`AccessToken`及对应的`RefreshToken`，确保认证状态被清除。用户后续需重新登录获取新的`AccessToken`和`RefreshToken`。

`OAuth 2.0`认证流程的时序图如下所示：

![1063b144-6ebb-4535-9eb3-e67941e6cc89](image/1063b144-6ebb-4535-9eb3-e67941e6cc89.png)
...
用户 -> Client: 访问客户端，申请获取资源服务器数据
Client -> AS: 将用户导向服务商的授权服务器，请求授权
AS --> 用户: 返回资源服务应用的授权页面，让用户授权
用户 -> AS: 用户进行授权
AS --> Client: 返回授权码到客户端应用
Client -> ClientBE: 将授权码传给服务器
...
ClientBE --> Client: 返回AccessToken

用户 -> Client: 发起业务请求
Client -> ClientBE: 携带AccessToken请求资源
ClientBE -> RS: 携带AccessToken请求资源
...
Client -> ClientBE: AccessToken失效，请求刷新
...
alt RefreshToken未过期
    ...
    ClientBE --> Client: 返回新的AccessToken
else RefreshToken已过期
    ClientBE -> DB: 删除user_token记录
    ClientBE --> Client: 要求重新向授权服务器请求授权码
end
```
