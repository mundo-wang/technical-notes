...
在`OIDC`流程中，通常会同时返回`ID Token`与`Access Token`。其中，`ID Token`用于向`RP`证明用户已完成身份认证，并携带用户身份相关的声明；`Access Token`则用于后续访问受保护的资源，其使用对象是`OP`提供的资源服务。例如，`RP`在获得用户授权后，如果还需要调用`OP`提供的用户详细信息、组织架构等受保护`API`，则应携带`Access Token`访问相应资源。

在本文介绍的企业内部`OIDC SSO`场景中，各业务系统作为`RP`接入统一身份中心，主要需求是完成用户身份认证并建立自身业务会话。如果业务系统仅需通过`ID Token`获取用户身份信息，无需进一步调用`OP`提供的受保护资源`API`，那么`Access Token`在整个单点登录流程中实际上并无直接的业务用途，也无需参与后续的业务请求。
...
3. `RP1`收到授权码和`state`后，首先校验`state`是否与本地保存的值一致，以确认该认证响应来自当前发起的登录流程。校验通过后，`RP1`通过后端服务向`OP`令牌端点发起请求，使用`client_id`、`client_secret`和授权码交换令牌。`OP`校验授权码有效性后，返回`ID Token`、`Access Token`以及可选的`Refresh Token`。
...
6. `RP2`收到授权码和`state`后，先校验`state`合法性，确认响应对应当前发起的认证请求。验证通过后，通过后端服务向`OP`交换令牌，使用`client_id`、`client_secret`和授权码获取`ID Token`、`Access Token`以及可选的`Refresh Token`。随后验证`ID Token`有效性，确认用户身份并创建业务会话。用户无需再次登录，即可访问多个接入同一`OP`的业务系统。

基于`OIDC`的单点登录核心时序流程如下图所示：

![0b425c4c-9662-44a3-8952-595540f0bc0b](image/0b425c4c-9662-44a3-8952-595540f0bc0b.png)
...
Administrator -> OP: 提交客户端注册申请\nclient_name + redirect_uri + scope + response_type
OP -> OP: 创建客户端配置\n保存RP客户端元数据
OP --> Administrator: 返回客户端信息\nclient_id + client_secret
Administrator -> RP1: 配置OIDC客户端信息\nclient_id / client_secret / redirect_uri
Administrator -> RP2: 配置OIDC客户端信息\nclient_id / client_secret / redirect_uri
...
alt 不存在本地会话
    ...
    OP --> RP1: 返回ID Token + Access Token + Refresh Token
...
alt 不存在本地会话
    ...
    RP2 -> OP: Token请求\nclient_id + client_secret + Authorization Code
    OP --> RP2: 返回ID Token + Access Token+ Refresh Token
