...
这里使用`%+v`而非`%v`，原因在于两者对结构体的处理方式不同。`%v`只打印各字段的`Value`，输出内容如下所示：

```sh
&{Value1 Value2 ...}
```

当字段较多时很难分辨每个`Value`对应的含义；`%+v`则会额外输出字段名，输出内容如下所示：

```
&{Field1:Value1 Field2:Value2 ...}
```

其可读性更高。`params.Set()`的返回值`response`是`*gosnmp.SnmpPacket`类型，属于结构体指针，`SnmpPacket`包含较多字段，使用`%+v`在调试时能直接看清每个字段对应的含义，定位问题更为方便。

其余类似`Walk、GetNext、GetBulk`操作都是一样的步骤，后续使用到再进行补充。