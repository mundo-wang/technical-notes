...
该模型专为文本向量化设计，输出`768`维向量，本地运行开销极小。
...
随后对每个文件按标题层级进行分块，将长文档拆解为若干语义完整的片段，代码如下：
...
// 将文本块连同向量和元数据批量写入Qdrant，id用自增序号。payload中存储原始文本和文件路径，检索后可直接组装Prompt
func upsertChunks(ctx context.Context, client *qdrant.Client, 
	collection string, chunks []Chunk, vectors [][]float32) error {
...
> 此处采用`Upsert`操作，以`id`为键执行插入或覆盖，但存在一类隐患：只要集合中某些旧`id`不会被新一轮写入命中，对应的块便会永久残留，形成脏数据。具体涉及以下场景：删除`.md`文件、在文件中删减内容导致分块数量减少、修改文件名或移动路径、以及调整`chunkSize`或`overlap`参数导致所有分块边界重新计算。在正式项目中应该改用更佳的方式。
...
func main() {
	...
	fmt.Printf("全部文件处理完成，共%d个文本块，开始写入Qdrant", len(allChunks))
	// 将所有文本块及其向量批量写入向量数据库
	err = upsertChunks(ctx, qdrantClient, collectionName, allChunks, allVectors)
	if err != nil {
		// 处理写入向量数据库过程遇到的错误
	}
	fmt.Printf("数据准备完成，共写入%d条记录", len(allChunks))
}
```
