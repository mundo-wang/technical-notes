...
推荐使用`Ollama`本地拉取`nomic-embed-text`作为`RAG Embedding`模型，命令如下：
...
接着我们使用`OpenAI`兼容接口的方式，调用`RAG Embedding`模型对每个片段进行向量化，将文本转换为高维浮点向量：
...
但由于本地部署的`LLM`与`RAG Embedding`模型能力均较弱，加之检索环节只有召回而没有重排，实际效果并不特别理想。
