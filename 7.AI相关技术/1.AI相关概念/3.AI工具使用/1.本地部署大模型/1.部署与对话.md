...
我的本机是搭载`M3 Pro`芯片的`MacBook Pro`，配备`18GB`统一内存与`512GB`固态硬盘。本次以采用`Q4_K_M`量化的`Qwen3-8B`为例进行本地部署，该模型文件大小约为`5GB`，在该配置下可以正常进行本地推理。推荐使用`Ollama`方案，该方案配置简单，并针对`Apple Silicon`提供了较好的原生支持。

首先，我们使用`Homebrew`安装`Ollama`：
...
通过`Ollama`运行的`qwen3:8b`对应`Q4_K_M`量化版本。`Qwen`官方的`Qwen3-8B-GGUF`仓库明确将`Q4_K_M`标记为`4-bit`量化，该量化模型文件大小约为`5.03GB`。实际运行时还需要额外的运行时内存和`KV Cache`，因此统一内存占用会高于模型文件大小，并会随上下文长度等因素变化。对于配备`18GB`统一内存的`M3 Pro`，运行该模型通常没有内存容量方面的问题。
...
如果接入的不是本地部署的大模型，而是第三方提供的云端`API`服务，例如`DeepSeek`，整体调用方式几乎无需修改。由于`DeepSeek API`兼容`OpenAI`接口规范，因此仍可继续使用`openai-go`进行开发，只需将客户端的`BaseURL`修改为`https://api.deepseek.com`，`APIKey`替换为平台申请的密钥，并将模型名称改为对应的云端模型（如`DeepSeek-V4-Pro`）即可。
