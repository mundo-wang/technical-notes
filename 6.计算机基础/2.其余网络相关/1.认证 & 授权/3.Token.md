...
在`OAuth 2.0`中，主要有两种类型的令牌：`Access Token`（访问令牌）和`Refresh Token`（刷新令牌），其概念如下：

- `Access Token`（访问令牌）：用于访问受保护资源的令牌，包含授权的权限信息，有效期通常较短，以降低安全风险。

- `Refresh Token`（刷新令牌）：当访问令牌过期时，用于获取新的访问令牌，通常比访问令牌的生命周期更长。

`OAuth 2.0`的授权过程包括以下步骤：

1. 申请授权码：客户端向认证服务器发送请求，用户完成认证后，认证服务器返回授权码。
2. 获取访问令牌：客户端使用授权码向认证服务器请求访问令牌和刷新令牌。
3. 使用访问令牌：客户端使用访问令牌向资源服务器请求受保护的资源。

为何需要`Refresh Token`？ 用户的权限也许会经常变化。如果只有`Access Token`，每次权限变化时都需要重新登录以生成新的访问令牌，过程繁琐。使用`Refresh Token`可以在权限发生变化时，使用刷新令牌生成新的访问令牌，从而避免了重复登录的麻烦。