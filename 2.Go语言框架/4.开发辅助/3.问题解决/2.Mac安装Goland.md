...
然后打开`Goland`，使用临时激活码或临时账号先激活软件（参考上面文档）。激活成功后，在`Goland`的上方栏选择`Help`，再选择`Edit Custom VM Options..`，并输入以下几行命令：
...
> 由于前述激活码方式已失效，可以使用下方账号密码登录`JetBrains`账号，系统将自动将软件激活至一年后：
>
> 账号：15902843360@189.cn
> 密码：buyaoxiugaimima
