...
我们对原`Gin`返回结构文件进行改造，代码还是放在`wresp/std_resp.go`中：
...
默认情况下，使用`NewErrorCode`创建错误码时，会自动将`HTTP`状态码设置为`500`。对于需要返回特定`HTTP`状态码的业务错误，例如未登录场景需要返回`401`，可以通过`NewErrorCodeWithStatus`函数创建对应的错误码：

```go
var (
    Unauthorized = wresp.NewErrorCodeWithStatus(
        10002,
        "未登录或权限不足，无法访问此资源",
...
此外，如果系统需要支持多语言，可以将错误信息由单一的`string`扩展为`map[string]string`，通过语言标识映射对应的错误文案，例如`zh-CN`、`en-US`等。这样无需随着语言数量增加而修改结构体字段，能够更灵活地扩展多语言支持。
