...
当响应体是`JSON`但结构不确定，或只需临时查看某几个字段时，可以将`T`指定为`map[string]any`，避免为此专门定义结构体：

```go
respWrapper, _ := whttp.NewGet[map[string]any]().WithTimeout(30*time.Second).WithBaseURL(baseURL).
    WithHeader("Authorization", "b6d4d1f9-69f5-4f2b-ac5c-621c589b7533").
    WithQueryParamByStruct(reqParams).Send()

resp := respWrapper.GetRespData() // map[string]any类型，可直接按key取值
fmt.Println(resp["code"], resp["message"])
```

如果只关心接口调用是否成功、不需要读取响应体中任何内容，可以将`T`指定为`struct{}`，`Send()`会尝试将响应体反序列化到空结构体，不会填充任何字段也不会报错，`GetRespData()`返回的是`struct{}{}`，通常直接忽略即可：

```go
respWrapper, _ := whttp.NewPost[struct{}]().WithTimeout(30*time.Second).WithBaseURL(baseURL).
    WithHeader("Authorization", "b6d4d1f9-69f5-4f2b-ac5c-621c589b7533").
    WithJsonBody(reqBody).Send()

// 只需判断err是否为nil，不需要使用respWrapper
```

注意`struct{}`仅适用于响应体是`JSON`的情况，若响应体是二进制或非`JSON`文本，`json.Unmarshal`会报错导致`Send()`失败。
