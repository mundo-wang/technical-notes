...
config.ErrorOutputPaths = []string{"/var/log/app/error.log"}
```
...
wlog.Error("Failed to xxx").Err(err).Field("name", "lisi").Log()
```

打印出的生产环境`JSON`格式日志如下所示：

```json
{"level":"ERROR","time":"2024-12-16 09:43:08","line":"test06/main.go:46","message":"Failed to xxx","error":"some errors","name":"lisi","caller":"main.CallSome"}
```
