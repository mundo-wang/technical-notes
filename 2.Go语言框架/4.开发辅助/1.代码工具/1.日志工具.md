...
无论是`Go`标准库中的`log`、`slog`，还是第三方库如`zap`，日志通常都会输出到标准错误`stderr`。这是因为在`Unix/Linux`系统的约定中，`stdout`用于承载程序的正常输出结果，而`stderr`则用于输出错误信息和诊断信息。
...
在代码中使用`fmt.Println`打印的内容会输出到标准输出`stdout`，而日志内容、错误信息等通常输出到标准错误`stderr`。需要注意的是，任意级别的日志默认都会输出到`stderr`，并非只有`Error`及以上级别的日志才输出到标准错误。
