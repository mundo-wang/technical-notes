...
```go
wlog.Msg("call xxx failed").Err(err).Field("xxx", "xxx").LevelError()
```

在这里，我们将日志级别方法放在调用链的最后，用以替代原本的`Log()`方法。这样在视觉和使用习惯上可以更直观地引导开发人员完整编写调用链，从而降低被忽略的风险，避免因遗漏而导致日志未打印的问题。

我们删除了`level.go`文件，并更新`entry.go`文件的代码内容，如下所示：
...
```go
wlog.Msg("call xxx failed").Ctx(ctx).Err(err).Field("xxx", "xxx").LevelError()
```
...
如果项目不依赖日志组件，而是直接将日志写入文件，并通过`tail`命令进行查看，那么在生产环境中就没有必要输出结构化日志。因此，可以将`loadZapConfig`方法调整为如下形式：
