...
```go
if err != nil {
    wlog.Error("call $ACTION$ failed").Err(err).Field("$FIELD$", $VALUE$)$NEXT$.Log()
	return $RETURN$ err
}$END$
```

如果是经过优化后的日志库，模板代码如下所示：

```go
if err != nil {
    wlog.Msg("call $ACTION$ failed").Err(err).Field("$FIELD$", $VALUE$)$NEXT$.LevelError()
	return $RETURN$ err
}$END$
```

由于我们的日志框架可以打印出函数名或者方法接收器+方法名的信息，所以在`$ACTION$`部分只需写明是调用哪个函数发生的`error`即可。返回值可能只返回一个`err`，也可能返回`nil, err`或者`"", err`，所以这里写的尽可能通用。

这里同样可以在`$ACTION$`部分，用简短语句说明具体失败的操作：

```go
if err != nil {
    wlog.Msg("Failed to $ACTION$").Err(err).Field("$FIELD$", $VALUE$)$NEXT$.LevelError()
	return $RETURN$ err
}$END$
```
...
如果发现光标的顺序不太对，可以点击下方按钮，在弹出的窗口中进行调整：

<img src="image/image-20260413162018410.png" alt="image-20260413162018410" style="zoom:40%;" />

在这里上下拖动可以调整顺序，还可以进行一些额外的配置：

<img src="image/image-20260413162201019.png" alt="image-20260413162201019" style="zoom:40%;" />
...
```go
wlog.Info("$MESSAGE$").Field("$FIELD$", $VALUE$)$NEXT$.Log()$END$
```
...
```go
wlog.Msg("$MESSAGE$").Field("$FIELD$", $VALUE$)$NEXT$.LevelInfo()$END$
```
