...
CREATE TABLE `ding_app_config` (
  ...
  `app_name` VARCHAR(128) NOT NULL COMMENT '钉钉应用名称',
  `app_icon` VARCHAR(128) NOT NULL DEFAULT '' COMMENT '钉钉应用图标地址',
...
表`ding_app_config`用于管理钉钉企业微应用的配置信息，每条记录对应一个微应用。字段`app_code`由后端生成，用于唯一定位一条数据。当前端无法或不能持有`corp_id`、`agent_id`、`ak`等信息时，可通过该字段实现定位，否则前端直接传入`corp_id`和`agent_id`，后端即可唯一定位到应用数据，无需`app_code`字段。

表`ding_user`用于记录系统用户与钉钉用户的关联关系，通过手机号建立关联，每个用户在每个企业下对应一条记录。

如果需要实现系统企业与钉钉企业的关联关系，可以新建下表：

```sql
CREATE TABLE `ding_corp_bind` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  ...
  `wecom_corp_id` VARCHAR(128) NOT NULL COMMENT '钉钉企业唯一id',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='钉钉企业绑定表';
```

这样，表`ding_corp_bind`与表`ding_app_config`之间形成了一对多的关系。

钉钉企业内部微应用免登接入系统平台的开发流程如下：

![ea155460-2097-47c4-94e0-f28037afdcec](image/ea155460-2097-47c4-94e0-f28037afdcec.png)
