...
同样地，企微应用表`wecom_app_config`也可以进行如下字段的扩展，修改表的`DDL`语句如下：

```sql
ALTER TABLE `wecom_app_config`
ADD COLUMN `tenant_id` BIGINT NOT NULL COMMENT '系统租户ID，关联sys_tenant.id' AFTER `id`,
ADD COLUMN `app_name` VARCHAR(128) NOT NULL COMMENT '企微应用名称' AFTER `app_code`,
ADD COLUMN `app_icon` VARCHAR(128) NOT NULL DEFAULT '' COMMENT '企微应用图标地址' AFTER `app_name`,
ADD COLUMN `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0=未授权，1=已授权' AFTER `corp_secret`;
```
