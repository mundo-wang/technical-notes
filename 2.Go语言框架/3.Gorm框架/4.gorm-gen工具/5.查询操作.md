...
尽量避免使用复杂的查询表达式。建议将复杂逻辑拆解为多个步骤，即使代码量有所增加，也要优先保证代码的可读性。
...
这种方式也存在一定的局限性。当过滤条件涉及模糊查询、大小比较运算、集合运算等非精确匹配时，该方法便不再适用。在这种情况下，可以先忽略该过滤条件，查询出结果集后再进行二次筛选。此外，对于零值字段，如果该字段的零值在业务中有实际意义，可能会导致该过滤条件被忽略，从而使查询结果出现错误。
