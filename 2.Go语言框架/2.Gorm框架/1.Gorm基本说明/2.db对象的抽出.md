...
我们使用`github.com/spf13/viper`这个包去读取配置文件，首先下载这个包：
...
这样写，在项目中的每个地方，想用到这个`db`对象时，都可以通过`utils.DB`获取它。
