...
这段代码中的`tx`对象是`*query.QueryTx`类型，其结构如下所示：
...
`Delete`方法默认必须包含删除条件，否则`Gorm`会阻止全局删除并返回`gorm.ErrMissingWhereClause`。如果业务确实需要执行全表删除，可以显式开启全局删除，如下所示：

```go
info, _ := usersQ.Session(&gorm.Session{AllowGlobalUpdate: true}).Delete()
```
...
`deleted_at`字段的设计存在潜在风险：若表中存在唯一索引字段（如用户名、手机号），即使执行软删除，这些字段的唯一约束仍然生效。此时，被软删除的数据依然占用唯一值，新数据在插入相同值时会触发唯一约束冲突，从而引发业务异常。因此，在实际设计中，应避免在同一张表中同时使用「唯一索引 + 软删除」的组合方案。
