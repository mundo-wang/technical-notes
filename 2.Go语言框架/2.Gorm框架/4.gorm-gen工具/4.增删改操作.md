...
在前文中，我们提到`Go`语言中的值类型`time.Time`对应`MySQL`的`datetime`类型。如果该字段未设置除`NULL`外的默认值，且插入数据时该字段未被初始化，默认会使用零值`0000-00-00 00:00:00`进行插入，这会导致数据库存储时报错。为了解决这一问题，可以使用`Omit`方法忽略`time.Time`类型的零值字段，使数据库能够正确存储默认的`NULL`值。示例如下：
