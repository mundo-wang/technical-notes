...
调用`Find`方法会返回所有符合条件的记录，而`Scan`方法则用于将查询结果集映射到指定对象中，该对象可以是基础类型、结构体类型，或它们的切片类型。

默认情况下，`Scan`会根据目标对象的字段名与表实体字段名自动进行不区分大小写的映射。如果需要自定义映射关系，可以在目标结构体字段加上`gorm:"column:xxxx"`标签，指定目标对象字段与表实体字段的对应关系。需要注意，字段类型必须能够兼容转换，否则映射会失败。此外，`Scan`方法的参数必须是引用类型，否则无法完成映射。
...
对于包含`deleted_at`字段且字段类型为`DATETIME`（在结构体中为`gorm.DeletedAt`类型）的表，上面的查询无法检索出`deleted_at`字段不为`NULL`的数据。如果想不过滤掉这些软删除的数据，可以使用`Unscoped()`方法，例如：
...
可以使用`Select`方法或`Omit`方法指定从数据库表中取出哪些字段，如果不加这个方法，默认为`select *`。`Select`方法一般配合`Scan`方法，把查询出的结果集扫描到指定对象中，代码如下：
...
当在`Select`方法的参数中仅涉及单个表的字段时，可以使用`Pluck`方法来简化操作，方法签名如下：

```go
Pluck(column field.Expr, dest interface{}) error
```

上述用于查询用户名称列表的代码，可以改写为如下形式：

```go
var nameList []string
usersQ.Where(usersQ.Email.Like("%gmail%")).Pluck(usersQ.Name, &nameList)
```
