...
实现时需要注意一个问题：过滤字段的零值是否具有业务含义。例如，一个`int`类型的字段`type`，当其值为`0`时代表特定业务语义，这种情况下就不能简单地以`0`来判断该字段是否应参与构造查询条件。这也反映了`Go`语言无法区分空值与零值的一个局限性。为了解决这个问题，我目前采用的做法是让前端在该字段无值时传入`-1`，以便后端据此判断是否添加过滤条件。

更稳妥的一种做法是将字段类型设置为基础类型的指针，例如`*int`。这样字段的值就可以为`nil`，我们就能明确区分出是未传值，还是传入了该类型的零值。在使用时，通过`*`解引用即可获取实际值。
