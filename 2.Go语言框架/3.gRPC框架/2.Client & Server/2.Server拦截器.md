在先前创建`gRPC Server`的代码中，是这样创建`gRPC`服务端的：`server := grpc.NewServer()`。
...
发现我们之前创建`gRPC`服务端时没有传入任何参数。`Gin`有一个中间件机制，允许请求进入处理器之前或离开处理器之后执行一些操作，例如日志记录、身份验证、错误处理等。`gRPC`有没有这样的机制呢？有的，`gRPC`的拦截器（`Interceptor`）机制允许在`gRPC`调用服务方法之前或之后插入自定义逻辑。这个拦截器的逻辑，就是在`NewServer`的函数参数中传递的。
...
1. 一元`RPC`：客户端发送单个请求到服务端，服务端接收请求，处理后返回一个响应给客户端。
2. 服务端流式`RPC`：客户端发送一个请求到服务端，服务端接收请求后，可以返回多个相应回给客户端。
3. 客户端流式`RPC`：客户端可以发送多个请求给服务端。服务端接收所有客户端请求，处理后发送一个响应回客户端。
4. 双向流式`RPC`：客户端和服务端可以同时发送消息和接收消息，适用于实时通信的场景。

我们写的普通的`gRPC`接口，都属于一元`RPC`，一元拦截器是处理一元`RPC`调用的中间件。在`Go`语言的`gRPC`服务端实现中，一元拦截器的类型是一个函数，定义如下：
...
要使用一元拦截器，需要通过`grpc.UnaryInterceptor`函数将自定义的一元拦截器注册到`gRPC`服务端，如下所示：
...
在注册`gRPC`服务端时，使用这样的方式注册这两个一元拦截器：
...
流拦截器适合处理服务端流式`RPC`、客户端流式`RPC`、双向流式`RPC`这几种方法类型，由于我接触的接口还没有涉及过这三种类型，所以流拦截器这里先不做讲解，以后再进行补充。
