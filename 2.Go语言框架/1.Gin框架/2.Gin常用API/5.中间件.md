...
中间件函数的命名应以`Middleware`结尾，并在前缀部分根据功能进行描述，以明确标识其为中间件函数。我们可以把所有的中间件函数都写到`middle/middleware.go`文件中。例如要记录每个访问请求的时间戳，代码如下：
...
在中间件链中，`c.Next()`用于将控制权传递给下一个中间件或路由处理函数，以继续执行后续逻辑。
...
> 如果中间件只需要执行前置逻辑，不涉及任何后置处理，可以省略在中间件函数末尾对`c.Next()`的调用。但如果中间件仅用于执行后置逻辑而没有前置逻辑，则必须将`c.Next()`的调用放在函数的首行。
...
```go
userGroup := r.Group("/v1/user", middle.LoggerMiddleware)

userGroup := r.Group("/v1/user")
userGroup.Use(middle.LoggerMiddleware)
```

如果需要对相同前缀路径下的不同接口使用不同的中间件，可以创建多个路由组对象，并分别为其指定对应的中间件：

```go
publicGroup := r.Group("/v1/user")                        // 公共路由组
userGroup := r.Group("/v1/user", middle.UserMiddleware)   // 普通用户路由组
adminGroup := r.Group("/v1/user", middle.AdminMiddleware) // 管理员路由组
```

这样一来，需要使用特定中间件的接口，就应当注册到对应的路由组对象下。

> 需要注意的是，在这里，中间件绑定的是对应的路由组对象，而非前缀路径本身。
