...
r.POST("/upload", func(c *gin.Context) {
    ...
    if err != nil {
        wresp.Fail(c, http.StatusBadRequest, fmt.Sprintf("get file error: %s", err.Error()))
        return
    }
    err = c.SaveUploadedFile(file, fmt.Sprintf("images/%s", file.Filename))
    if err != nil {
        wresp.Fail(c, http.StatusInternalServerError, fmt.Sprintf("save file error: %s", err.Error()))
        return
    }
    wresp.OK(c, nil)
})
...
```go
ext := filepath.Ext(file.Filename) // 获取的后缀不包含点（.）
```

例如输入`caixukun.jpg`，可以提取出`jpg`后缀。提取后缀后，可以自定义拼接文件名，并将其保存到`images/`目录下。
...
r.POST("/uploadMultiple", func(c *gin.Context) {
    ...
    if err != nil {
        wresp.Fail(c, http.StatusBadRequest, fmt.Sprintf("obtaining form error: %s", err.Error()))
        return
    }
...
上传文件接口想在获取文件后、保存文件前读取文件内容，可以在接口中加入以下代码：

```go
// 此处代码以上是从请求中获取文件逻辑
src, err := file.Open()
if err != nil {
    wresp.Fail(c, http.StatusInternalServerError, fmt.Sprintf("open file error: %s", err.Error()))
	return
}
defer src.Close()
fileBytes, err := io.ReadAll(src)
if err != nil {
    wresp.Fail(c, http.StatusInternalServerError, fmt.Sprintf("read file error: %s", err.Error()))
	return
}
