...
表单格式主要包括`multipart/form-data`和`x-www-form-urlencoded`两种。`multipart/form-data`既支持传输文本数据，也支持传输文件等二进制数据，并允许为每个字段附加额外信息；而`x-www-form-urlencoded`仅支持传输纯文本数据。从技术实现角度来看，完全可以统一使用`multipart/form-data`格式来上传所有表单数据。

上传表单参数，必须使用`POST`方法，请求数据以表单格式存放在请求体中。在`Apifox`中，该类型对应为`form-data`：

<img src="image/image-20250722093446029.png" alt="image-20250722093446029" style="zoom:50%;" />

可以使用`PostForm`方法获取字符串格式的表单参数：
...
使用另一个方法`FormValue`也可以获取字符串格式表单参数：

```go
c.Request.FormValue("user-name")
```

`FormValue`方法可以处理`GET`或`POST`请求，读取`URL`中的查询参数或请求体中的表单参数。如果使用`POST`请求，并且`URL`查询参数和请求体表单参数中有同名参数，它会优先读取`URL`查询参数的值。

关于获取表单上传的文件的相关内容，请参考后文的“文件上传下载”章节。
...
`c.Query()`、`c.PostForm()`、`c.Param()`和`c.GetHeader()`获取到的都是字符串`string`类型。
...
从`HTTP`规范的角度来看，`GET`方法并不禁止携带请求体，`c.ShouldBind`方法也可以获取到`GET`请求的请求体内容，但一些中间件、代理可能会忽略`GET`请求的请求体，因此如果需要传递请求体参数，需要注意避免使用`GET`方法。
