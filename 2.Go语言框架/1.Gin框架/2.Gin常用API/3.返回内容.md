...
> 应避免使用`c.JSON`方法返回字符串，否则可能会导致一些难以排查的问题，之前接手其他人代码踩过此坑。
...
除了直接向前端返回响应数据之外，还有一类接口，服务端并不返回具体的业务数据，而是指示客户端跳转到另一个地址。这种行为称为重定向，在`HTTP`协议中，重定向的实现方式本质上是`3`开头状态码配合`Location`响应头来完成。当客户端收到包含重定向状态码和`Location`头的响应后，会根据该头部中指定的地址，自动向新的目标地址重新发起请求。

在`Gin`框架中，提供了专门的方法用于处理重定向，即`c.Redirect`，其方法签名如下：

```go
func (c *Context) Redirect(code int, location string)
```

其中，第一个参数表示状态码，例如`301`、`302`等；第二个参数表示跳转的目标地址。若为跨站点跳转，需要填写完整的目标地址；若为站点内跳转，可以只填写接口路径，客户端会基于当前请求的协议、域名以及端口信息，自动将其拼接为完整地址。

该方法的使用示例如下，例如重定向到`/login`接口：

```go
c.Redirect(http.StatusFound, "/login")
```

如果不使用`c.Redirect`，也可以通过手动设置状态码和响应头`Location`来实现重定向：
...
这种方式本质上与`c.Redirect`等价，但可读性和可维护性较差，一般不建议使用。

在发生重定向时，服务端不会返回任何响应体。因此，在调用`c.Redirect`之后，应当立即执行`return`，以避免后续业务逻辑继续运行。在中间件中使用`c.Redirect`之后，应立即调用`c.Abort`，以阻止请求继续向后续处理链传递。
