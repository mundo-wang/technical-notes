...
在`Controller`层直接打印`Service`层返回的`error`并不合理。因为错误码定义的`error`通常源于用户的不当操作，若遭遇恶意攻击，可能会导致系统生成大量`ERROR`级别的日志，干扰正常监控。因此，可以使用`IsErrorCode`函数进行判断：对于业务错误，不记录日志；对于系统错误，则记录日志，以确保系统错误的可追溯性。
...
func ReportError(err error, operation string, fields map[string]any) {
	...
	l := wlog.Errorf("call %s failed", operation).Err(err).Skip(2)
...
这样就可以在`Controller`层调用该函数处理打印日志的逻辑：
...
func (h *UserHandler) CreateUser(c *gin.Context) (any, error) {
	...
	err := h.UserService.CreateUser(user)
	if err != nil {
		utils.ReportError(err, "h.UserService.CreateUser", map[string]any{
			"user":  user,
		})
		return nil, err
	}
...
最佳实践是仅当`Service`层有错误码定义的`error`返回时，才在`Controller`层进行`IsErrorCode`判断，否则统一打印日志。后续若在`Service`层新增了错误码定义的`error`返回，再在`Controller`层补充对应的判断逻辑。
