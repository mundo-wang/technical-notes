对于pgsql，也可以使用explain查看其执行计划，但其显示出的结果与MySQL不同。
...
| `cost=0.00..10.10` | 表示执行该查询的成本估算范围从0.00到10.10。 |
| `rows=10`          | 表示查询的结果集中预计包含10行。            |
| `width=5217`       | 表示结果集的每一行的宽度为5217个单位。      |
...
| `cost=0.14..8.15`      | 表示执行该查询的成本估算范围从0.14到8.15，成本值越低，查询速率越快。 |
| `rows=1`               | 表示查询的结果集中预计包含1行，这是由于使用了主键，所以只能匹配一行。 |
| `width=5217`           | 表示结果集的每一行的宽度为5217个单位。                       |
| `Index Cond: (id = 2)` | 表示使用了索引条件，其中 `(id = 2)` 表示对主键为2的行进行检索。 |
...
这里要注意，如果你使用`ANALYZE`关键字的话，它会**执行一遍命令**，为了显示实际执行的行数、执行时间等详细信息，这点和正常我们理解的 explain 命令不会具体执行SQL语句不同。