...
`go get`后面跟的是目标依赖的模块路径，即该库自身`go.mod`文件中`module`字段所声明的值，如下所示：

```sh
go get github.com/mundo-wang/wtool
```

`go get`命令一方面用于下载或更新第三方`Go`代码包，并将其存放在`$GOPATH/pkg/mod`目录下；另一方面，它会将这些依赖添加到当前`GoModules`项目的`go.mod`文件中，并在`GoLand`中显示为外部依赖库，如下所示：
...
> **注意：** 使用`go get -u`命令时需谨慎，`-u`参数不仅会更新指定的依赖库，还会递归升级其完整依赖树中的所有间接依赖，导致影响范围难以预估。因此，使用该命令时需注意潜在的版本冲突，以及依赖升级后与本地`Go`环境不兼容的问题。
>
> 在`Go 1.21`之后，`go get`在更新依赖时，如果目标依赖的`go.mod`中声明的`go`版本高于当前项目的`go`版本，工具链会自动将当前项目`go.mod`中的`go`字段提升，以满足依赖的最低版本要求。
>
> 例如当前项目声明`go 1.21`，拉取的依赖声明`go 1.23`，执行完毕后项目的`go.mod`会被自动改写为`go 1.23`。

在实际开发中，如果只想更新指定的依赖库而不影响其间接依赖，可以使用`@latest`语法，如下所示：

```sh
go get github.com/mundo-wang/wtool@latest
```

相比`go get -u`，该命令仅将指定库升级到最新版本，不会递归升级其间接依赖，从而有效规避依赖升级引发的潜在兼容性问题。
...
如果不想拉取默认分支，而是想拉取指定分支的最新提交，可以使用以下命令：

```sh
go get github.com/mundo-wang/wtool@feature
```
