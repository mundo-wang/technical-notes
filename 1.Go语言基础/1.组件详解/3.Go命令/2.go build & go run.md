...
- `2>&1`：将标准错误（`stderr`，即文件描述符`2`）重定向到标准输出，这样标准错误的信息也会写入`output.log`文件。
- `&`：使程序在后台运行。

使用`>`重定向会覆盖文件内容，如果想在文件中追加输出，应使用`>>`：
...
如果只想将标准错误写入文件，而不重定向标准输出，让其依然输出到终端，可以使用如下命令：

```sh
nohup go run main.go 2> error.log &
```

如果希望忽略标准输出在终端的输出，可以加上`/dev/null`进行丢弃：

```sh
nohup go run main.go > /dev/null 2> error.log &
```
