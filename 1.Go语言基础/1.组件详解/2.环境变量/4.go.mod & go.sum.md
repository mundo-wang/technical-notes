...
从`Go 1.21`开始，`go.mod`文件新增了`toolchain`关键字，用于指定模块编译时使用的`Go`工具链版本。在示例文件中，`go`行指定了模块最小可用`Go`版本为`1.21.0`，`toolchain`行指定实际编译时使用的工具链版本为`1.21.4`。这一特性允许模块在运行时兼容较低版本的`Go`，但在编译时使用更高版本的工具链，以确保新版本的特性和功能支持。
