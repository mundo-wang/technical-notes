...
在`go.mod`文件中，可以使用`go 1.xx.x`或`go 1.xx`两种格式指定`Go`版本。

`Go 1.20`之前，官方推荐使用次要版本号（如`go 1.20`），而非具体的补丁版本号（如`go 1.20.8`）。使用`go mod init <module-path>`命令生成`go.mod`文件时，生成的`go`版本通常包含补丁版本号，建议手动去除补丁号，以保持简洁并符合官方推荐。
...
如果使用高于`go.mod`声明版本的`Go`进行开发，团队协作中可能出现兼容性问题。例如，项目的`go.mod`声明`go 1.20`，但某位开发者使用了`Go 1.21`的特性编写代码，那么这部分代码会无法被仍使用`Go 1.20`的其他开发者正常编译。

因此，在团队协作时，建议所有开发者统一`Go`版本，以避免兼容性问题。

此外，项目依赖的模块也可能要求更高版本的`Go`。从`Go 1.21`开始，主模块声明的`go`版本必须大于或等于所有依赖模块声明的`go`版本。如果新增依赖要求`Go 1.21`，而当前项目声明为`go 1.20`，那么`Go`会要求主模块提升其`go`版本。

#### 3. `toolchain`关键字

从`Go 1.21`开始，`go.mod`文件新增了`toolchain`关键字，用于指定模块编译时使用的`Go`工具链版本。

在上面的示例文件中，`go`行声明该模块的最小兼容`Go`版本为`1.21.0`，意味着其他项目依赖该模块时，应使用`Go 1.21.0`或更高版本。`toolchain`行则声明该模块期望使用的工具链版本为`1.21.8`，即在该模块内进行编译或测试时，会强制使用`Go 1.21.8`的工具链。如果`go.mod`文件中没有`toolchain`行，则默认`toolchain`的`Go`版本与`go`行声明的版本相同。

`toolchain`解决的核心问题是，当本地全局`Go`版本较低，而项目或模块依赖更高版本的`Go`语法或特性时，无需升级本机`Go`环境，只需在`go.mod`中声明`toolchain`，`Go`即可自动下载并使用对应版本的工具链进行编译与运行，从而兼容高版本依赖及新语法。

同时，它还能统一团队与`CI`环境中的编译工具链版本，避免因本地`Go`版本不一致而导致的编译差异或隐蔽问题。

如果本地`Go`版本低于`toolchain`指定的版本，`Go`会自动下载并使用指定版本的工具链。版本名称格式如下所示：

```sh
toolchain@v0.0.1-go<VERSION>.<GOOS>-<GOARCH>
```

工具链以目录形式存放，存储在`$GOPATH/pkg/mod/golang.org`路径下：

<img src="image/image-20250412230024943.png" alt="image-20250412230024943" style="zoom:40%;" />

当使用`go get`命令下载的模块所指定的`toolchain`版本高于本地的版本时，`Go`会自动下载新版本工具链，保存到上述路径下，并自动使用。同时，它会更新项目`go.mod`文件中的`toolchain`字段。
...
在`Go 1.21`版本中新增了环境变量`GOTOOLCHAIN`，可以通过`go env GOTOOLCHAIN`查看其值，默认值为`auto`。在`auto`模式下，`Go`会自动决定使用哪个工具链。具体而言，当`go.mod`文件中指定的`toolchain`版本低于本地的`Go`版本时，使用本地的`Go`版本；如果`go.mod`文件中指定的`toolchain`版本高于本地的`Go`版本，则会自动下载并使用指定版本的工具链。

除了`auto`，`GOTOOLCHAIN`还可以设置为`local`，表示始终使用本地的`Go`自带的工具链，不允许自动下载或切换到其他版本的工具链。如果`go.mod`文件中指定的`toolchain`版本高于本地的`Go`版本，则会报错。

如果本地`Go`版本低于项目要求的版本，但`Go`没有自动下载对应的`toolchain`，且已确认`GOTOOLCHAIN=auto`，此时需要对`GOSUMDB`配置进行检查。`Go`自动下载的工具链校验并不依赖项目的`go.sum`，而是通过`GOSUMDB`完成验证。`GOSUMDB`用于验证下载内容的完整性，如果将`GOSUMDB`设置为`off`，会导致工具链无法完成校验，从而导致自动下载失败。

因此，通常应保持`GOSUMDB`为默认值`sum.golang.org`。在国内网络环境下，则可以根据实际网络情况配置可访问的模块代理，同时确保代理能够正常提供`golang.org/toolchain`模块。
...
我们一般看到的`go.mod`文件中有两个`require`块：

1. 第一个块列出的依赖是直接依赖，也就是我们的代码直接`import`的模块。
2. 第二个块列出的依赖后面带有`// indirect`注释，这表示间接依赖。

具体来说，如果模块`A`依赖模块`B`，而我们的代码只`import`了模块`A`，没有直接`import`模块`B`，那么`B`会被`go`记录为间接依赖，在`go.mod`中显示为带`// indirect`的依赖。

间接依赖记录的目的主要是为了保证模块`A`在不同环境或版本下依赖的`B`版本保持一致，避免构建出错。
