...
在`go.mod`文件中，可以使用`go 1.21.2`或`go 1.21`两种格式指定`Go`版本，但官方推荐使用次要版本号（`go 1.21`），而不是具体的补丁版本号（`go 1.21.2`）。使用`go mod init <module-path>`命令生成`go.mod`文件时，其中的`go`版本会包含补丁版本号，建议手动去除补丁号，以保持简洁和符合官方推荐。
...
`go.mod`文件还有`replace`（替代模块）、`exclude`（排除模块），以及`toolchain`（工具链，`go 1.21`后引入），它们是可选的。
