...
> 如果没挂梯子，上面的网站可能打不开，也可以访问：https://golang.google.cn/dl/。

### 一、官网下载`Go`环境（建议）

首先使用`uname -m`命令查看一下自己` MacBook `的`CPU`架构：
...
<img src="image/image-20240201135700280.png" alt="image-20240201135700280" style="zoom:40%;" />

> 在`2020`年以前，搭载`Intel Core`系列处理器的`MacBook`所采用的`CPU`架构为`x86_64`，而非`arm64`。

下载完成后，双击运行安装程序，按照提示一路点击「下一步」即可。安装目录（`GOROOT`）建议使用默认值`/usr/local/go`。无需手动将`/usr/local/go/bin`添加到环境变量，安装过程会自动完成该配置。
...
添加完成后，使用`source ~/.zprofile`或者`source ~/.zshrc`刷新配置文件，即可使用`go version`命令查看到版本信息。

> 每次打开一个新的终端窗口或标签页，就会启动一个新的交互式`Shell`，此时`.zshrc`就会被执行一次。关闭该终端后，这个`Shell`进程结束，下次再开新窗口时会再执行一次。

`.zprofile`与`.zshrc`的核心差异在于加载时机，由`Shell`的登录与交互两个维度共同决定：

- 登录式交互`Shell`，如`SSH`连入或`macOS`终端首次打开，会依次加载`.zprofile`与`.zshrc`；
- 非登录式交互`Shell`，如在已有终端中新开标签页，仅加载`.zshrc`；
- 登录式非交互`Shell`，如某些脚本执行环境，仅加载`.zprofile`；
- 非登录式非交互`Shell`，如直接运行`zsh script.sh`，两者均不加载，但`.zshenv`仍会生效。

实际开发中，把环境变量放`.zprofile`、把交互配置放`.zshrc`是比较清晰的做法，但全放在`.zshrc`也不会出问题。
