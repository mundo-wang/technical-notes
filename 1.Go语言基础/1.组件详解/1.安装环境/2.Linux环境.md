...
2. 删除相关环境变量配置。在`Linux`环境下，`GOROOT`和`GOPATH`的相关环境变量配置通常存储在`/etc/profile`文件中。而在有着`zsh`终端的`Mac`环境下，这些配置通常位于`~/.zshrc`或`~/.zprofile`文件中的一个。
3. 删除`GOPATH`目录，在`Linux`环境下通常是`/home/<用户名>/go`（普通用户）或`/root/go`（`root`用户），在`Mac`环境下通常是`/Users/<用户名>/go`下面（若未修改）。
