...
在`Windows`系统中，首先确认自己电脑的`CPU`架构版本，例如系统是`x86-64`版本，则应选择`amd64`版本的安装包。在下载页面中，找到以`.msi`为后缀的文件，并进行下载。这里我们选择`go 1.21.2`版本：
...
当模块在代理上可用时，`Go`会优先从代理下载，不会直接访问`GitHub`或其他源仓库。代理本质上是对`GitHub`、`GitLab`等公共仓库的模块进行缓存和打包，获取到的仍然是原始源码，只是通过代理间接获取，而非直接访问仓库。

当代理上没有模块，或者代理访问失败时，`Go`会根据回退策略（`direct`）直接访问源码仓库，例如：

- 对于`github.com`上的模块，`Go`会使用`git`拉取对应版本的源码。
- 如果访问受限或是私有仓库，需要配置相应的`SSH Key`或访问`Token`，否则会拉取失败。
