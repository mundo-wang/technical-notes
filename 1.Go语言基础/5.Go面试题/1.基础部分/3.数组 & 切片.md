...
运行程序，打印结果为`[a b c d e]`。

这是因为切片的本质是由指针、长度、容量组成的结构体。当切片`s`被传入函数`addElement`时，传入的是这三个字段的拷贝。在示例中，`addElement`中的切片`s`的`cap`为`5`，当需要追加新元素`f`时，原有容量不足，`append`会创建新的底层数组并拷贝原内容，也就是切片扩容。这样，函数内部的`s`会指向新的底层数组，且`len`变为`6`；而函数外的`s`依旧指向原来的底层数组，且`len`与`cap`保持不变，因此最终输出仍为原有内容。
...
这是因为，函数`addElement`内部的切片`s`在执行`append`后，其`len`已经变为`6`，但函数外的切片`s`仍然保持长度`5`，因此即使函数内外的`ptr`指向同一个底层数组，外部打印时只会输出其`len`范围内的元素。
...
切片的切片语法本质上是基于已有数组或切片创建一个新的切片。新切片不会拷贝底层数据，而是与原数组或切片共享同一个底层数组，只是通过新的`ptr`、`len`和`cap`来描述自身可访问的范围。切片语法表示如下：

```go
newSlice := baseSlice[low:high]
```

`low`表示起始索引（包含），`high`表示结束索引（不包含），它们都从下标`0`开始计算。新切片的长度为`high - low`，容量为`cap(baseSlice) - low`；如果不填写`low`，默认为`0`，不填写`high`，默认为`len(baseSlice)`。

使用示例如下所示：

```go
baseSlice := []int{1, 2, 3, 4, 5} // len=5, cap=5
s1 := baseSlice[1:3]              // len=2, cap=4
s2 := baseSlice[:4]               // len=4, cap=5
s3 := baseSlice[2:]               // len=3, cap=3
s4 := baseSlice[:]                // len=5, cap=5
```

`Go`语言的切片的切片，其索引一律不能为负数，这是语言层面直接禁止的，不存在任何语法糖。

`high`的取值不能超过`cap(baseSlice)`，否则会触发运行时错误，如下所示：

```go
s5 := baseSlice[1:6] // panic: runtime error: slice bounds out of range [:6] with capacity 5
```

需要注意的是，`high`的取值可以大于`len(baseSlice)`，如下所示：

```go
baseSlice := make([]int, 3, 5) // len=3, cap=5
s1 := baseSlice[1:4]           // len=3, cap=4
```

`Go`还支持一种更精细的三索引切片语法，用于显式限制新切片的容量：

```go
newSlice := baseSlice[low:high:max]
```

这种切片方式下，新切片的长度仍然为`high - low`，但容量会变为`max - low`。代码如下所示：

```go
baseSlice := make([]int, 3, 5) // len=3, cap=5
s1 := baseSlice[1:3:4]         // len=2, cap=3
```

这种用法要求必须满足`0 <= low <= high <= max <= cap(baseSlice)`。
