...
为了避免相同的明文块在使用固定密钥加密时生成相同的密文块，通常推荐采用`CBC`模式。该模式会引入一个初始化向量（`IV`），从而确保相同的明文在加密后得到不同的结果。
...
对称加密的优势在于计算速度快、执行效率高，非常适合用于加密大体积数据。但在客户端与服务端首次通信等开放式网络环境中，由于通信双方尚未建立可信关系，对称加密的密钥难以实现安全传输。同时，密钥在多客户端管理、分发以及安全共享方面也存在较高复杂度，需要被格外慎重地设计与处理。
