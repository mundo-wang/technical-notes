...
另一个更严重的问题是，`CBC`依赖`PKCS7`填充，在服务端会对外暴露填充错误信息的场景下，极易遭受填充预言攻击，攻击者通过反复构造密文并观察响应差异，可逐字节还原出完整明文，历史上的`POODLE`、`BEAST`漏洞均与此相关。当前推荐的替代方案是`AES-GCM`，它属于`AEAD`模式，加密与完整性校验一步完成，无需填充，且支持硬件并行加速，`TLS 1.3`已强制要求使用该模式。
...
与`AES`等分组加密算法不同，`RSA`不对明文进行分组，而是直接加密有限长度的数据，其加密长度受密钥长度限制。具体而言，最大加密数据长度为`key_length - padding_length`字节，其中`padding_length`为填充开销，这里为`11`。由于`RSA`加密效率较低，它常与对称加密结合使用，例如使用`RSA`加密对称密钥，再使用`AES`加密数据。
