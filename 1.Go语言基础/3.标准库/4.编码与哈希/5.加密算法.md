...
与`AES`等分组加密算法不同，`RSA`不对明文进行分组，而是直接加密有限长度的数据，其加密长度受密钥长度限制。具体而言，最大加密数据长度为`key_length - padding_length`字节，其中`padding_length`为填充开销，这里为`11`。由于`RSA`加密效率较低，它常与对称加密结合使用，如用`RSA`加密对称密钥，再用`AES`加密数据。
