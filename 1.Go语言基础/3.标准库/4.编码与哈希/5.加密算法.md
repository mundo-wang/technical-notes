...
加密算法用于将信息从可读的形式转换为不可读的形式，以保护数据的隐私和安全。加密算法通过密钥将明文数据转换为密文数据，从而防止未经授权的访问或窥探，只有持有相同密钥的人才能解密数据，恢复其原始的明文内容。
...
`AES`是一种分组加密算法，它会将明文划分为多个`128`位（即`16`字节）大小的分组，并对每个分组分别进行加密。`AES`的密钥长度支持`128`、`192`和`256`位三种，分别对应`16`、`24`和`32`字节。具体的工作原理，这里不做详细讲解。

为了避免相同的明文块在使用固定密钥加密时生成相同的密文块，通常推荐在`AES`的基础上采用`CBC`模式。该模式会引入一个随机的初始化向量`IV`，从而确保相同的明文在加密后得到不同的结果。

> 这里的`IV`类似于盐，都是通过引入随机性来防止相同输入产生相同输出。
>
> 但需要注意的是，盐属于哈希领域，用于不可逆的单向运算，其核心目的是抵御彩虹表与预计算攻击，让攻击者无法用提前制好的查找表批量还原密码；`IV`属于加密领域，用于`CBC`等分组模式的可逆加密中，作为链式运算的起点直接参与密码学计算，核心目的是消除密文中的块重复模式，防止攻击者在不解密的情况下从密文结构中推断出明文信息。
>
> 此外，两者的重用后果严重程度也不对等，盐重复使用的危害相对有限，而在相同密钥下重复使用`IV`则可能导致攻击者通过比对密文差异逐步还原部分明文，安全风险更高。

在`Go`语言中，可以借助标准库中的`crypto/aes`和`crypto/cipher`包来实现`AES-CBC`模式的加解密操作。整体流程如下：
...
在执行加密之前，首先会对原始明文数据进行`PKCS7`填充，其函数如下所示：
...
在解密步骤完成后，去除明文的`PKCS7`填充，恢复原始明文，其函数如下所示：
...
对称加密的优势在于计算速度快、执行效率高，非常适合加密大体积数据。

但在客户端与服务端首次通信等开放式网络环境中，通信双方尚未建立可信关系，密钥难以安全传输。此外，在多客户端场景下，密钥的管理、分发与共享同样面临较高的工程复杂度，需要格外审慎地设计与处理。

`AES-CBC`目前已不推荐在新系统中使用。它只提供机密性，不提供完整性保护，攻击者无需破解密钥即可对密文进行比特翻转，悄无声息地篡改解密后的明文；若要弥补这一缺陷，还需额外引入`HMAC`，工程复杂度较高，也容易出错。

另一个更严重的问题是，`CBC`依赖`PKCS7`填充，在服务端会对外暴露填充错误信息的场景下，极易遭受填充预言攻击，攻击者通过反复构造密文并观察响应差异，可逐字节还原出完整明文，历史上的`POODLE`、`BEAST`漏洞均与此相关。

当前推荐的替代方案是`AES-GCM`，它属于`AEAD`模式，加密与完整性校验可一步完成，无需填充，且支持硬件并行加速，`TLS 1.3`已强制要求使用该模式。

### 2. 非对称加密

非对称加密采用一对密钥，分别为公钥和私钥。其中，公钥用于加密数据，可以对外公开，任何人都可以使用它对数据进行加密；而私钥用于解密数据，由接收方私密保管，只有持有对应私钥的一方才能成功解密。
...
`RSA`算法的安全性基于大整数因式分解的数学难题。它使用一对密钥，公钥用于加密、私钥用于解密；反过来，私钥用于数字签名、公钥用于验签。常见的密钥长度有`1024`、`2048`和`4096`位，密钥越长安全性越高，但性能开销也随之增大。

与`AES`等分组加密算法不同，`RSA`不对明文进行分组，而是直接加密有限长度的数据，其加密长度受密钥长度限制。具体而言，最大加密数据长度为`key_length - padding_length`字节，其中`padding_length`为填充开销，这里为`11`。

`PKCS1v15`负责对明文进行填充，至少填充`11`字节，填充后的明文长度会补齐到正好等于模长度`n`，从而可以进行`RSA`加密。该算法不能保证相同明文每次生成不同密文，如果需要更高安全性并确保相同明文每次加密得到不同密文，推荐使用`RSA-OAEP`填充，它采用更强的随机化机制和哈希函数，能够有效防止密文重复及多种攻击。
...
func rsaEncrypt(plainText string, publicKeyPem []byte) (string, error) {
	...
	// rest字段作用：返回输入数据中除已解析出的首个PEM块外的剩余字节内容
...
非对称加密无需提前共享密钥，通信更灵活，安全性也较高。代价是计算复杂，性能显著弱于对称加密，且对加密数据的长度有限制，生成的密文通常也更长，因此更适合用于加密小体积的敏感信息。

实际应用中，`RSA`常与对称加密配合使用，用`RSA`加密对称密钥、用`AES`加密数据，可兼顾安全性与效率。
