...
var (
	...
	usedIdentifiers = map[string]struct{}{} // 保存已处理请求的唯一标识符
)

// 中间件，前置操作，负责验证请求的签名
func CheckSignatureMiddleware(c *gin.Context) {
    // 请求头参数的命名规则：每个单词的首字母需大写，单词之间使用中划线（-）分隔
	ak := c.GetHeader("Access-Key")
	...
	c.Request.Body = io.NopCloser(bytes.NewBuffer(body)) // 将Body数据重新写到请求体，以便再次读取
...
// 使用HMAC-SHA256算法，构造签名字符串，生成签名，验证签名是否一致
func verifySignature(ak, sk, identifier, timestamp,
	receivedSignature string, body []byte, queryParams url.Values) bool {
...
func main() {
	r := gin.Default()
	r.Use(CheckSignatureMiddleware) // 设置验证请求签名中间件
...
// 使用HMAC-SHA256算法，按照服务端的约定，构造签名字符串，生成签名
func generateSignature(ak, sk, identifier,
	timestamp string, body []byte, queryParams url.Values) string {
	var signatureBuilder strings.Builder
	signatureBuilder.WriteString(ak)
	signatureBuilder.WriteString(identifier)
	signatureBuilder.WriteString(timestamp)
