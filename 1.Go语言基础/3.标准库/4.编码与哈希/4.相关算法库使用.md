...
使用`SHA-256`算法需要引入`crypto/sha256`包。首先，我们创建一个新的哈希对象：
...
盐值应与哈希后的数据一同存入数据库，并建立关联，以便后续使用相同盐值重新哈希数据，验证其一致性。

在前面我们讲到了`HMAC`与`SHA-256`的结合使用，这里我们引入`crypto/hmac`这个库，并定义一个密钥：
