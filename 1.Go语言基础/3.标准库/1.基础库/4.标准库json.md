...
`Go`语言并未提供类似的格式指定方法。`encoding/json`在序列化时，会根据字段类型和`tag`自动转换为固定格式。`time.Time`类型字段默认会被序列化为`ISO 8601`格式，即`2006-01-02T15:04:05.9999999Z07:00`，例如`2025-07-07T17:25:31.9130175+08:00`。
