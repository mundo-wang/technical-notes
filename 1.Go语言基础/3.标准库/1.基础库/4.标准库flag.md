...
在输入带值的`flag`时，可以通过空格或`=`与参数值进行连接，例如`-name John`与`-name=John`在语义上完全等价。对于布尔类型的`flag`，如果未显式指定取值，则默认值为`true`，例如`-married`等价于`-married=true`。
...
对于非布尔类型的参数，如果我们仅指定参数但是没有提供对应的值，如下所示：

```sh
go run main.go -name John -age -married
```

程序在解析命令行参数阶段就会直接报错并调用`os.Exit(2)`退出，打印结果如下：

```sh
mundo@Mundos-MacBook-Pro my-prac % go run main.go -name John -age -married
invalid value "-married" for flag -age: parse error
Usage of /Users/mundo/Library/Caches/go-build/3a/3a22dd8f43cd68aaa07d296fbfd2968ca17a941613e8465de43057f01489b52c-d/main:
  -age int
        Your age (default 20)
  -married
        If married, add this parameter
  -name string
        Your name (default "Unnamed")
exit status 2
```

我们以`flag.StringVar`函数为例进行讲解，其函数签名如下所示：
