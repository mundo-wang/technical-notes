...
众所周知，`Go`语言中的`map`并非并发安全结构，在并发读写场景下容易发生数据竞争，从而导致程序行为不可预期。

那么如果想在并发环境下使用`map`，都有哪些保证并发安全的方式？
...
使用`Load`方法读取`sync.Map`中的数据后，可以通过类型断言将其还原为存入时的具体类型：
```go
result := ""
res, ok := sm.Load(8)
if ok {
...
判断`ok`是否为`true`是必要的。当`Map`中不存在指定的`Key`时，返回的`value`为`nil`，若直接进行类型断言，将触发`panic`。
...
对于全局的`map`对象，必须配套使用锁以保证并发安全。对于局部的`map`对象，则可以根据具体使用场景决定是否加锁：如果会被多个`Goroutine`并发访问，且存在写操作，则必须加锁；如果不存在并发读写，则可以不加锁。需要注意的是，如果某个全局`map`实际上只在单个`Goroutine`中使用，最佳实践是将其调整为局部变量，缩小作用域，以降低并发复杂度并提升代码可维护性。

### 5. 遍历`map`的注意事项

获取常规`map`对象的单个`Key`对应的`Value`时，可以选择只接收一个返回值，也可以接收两个返回值。
...
这种写法只接收`Value`。如果`Key`存在，返回对应的`Value`，如果`Key`不存在，返回该`Value`类型的零值。
...
第二个返回值`ok`是一个`bool`类型变量，用于表示`Key`是否存在。当`ok == true`时，说明`Key`存在，此时返回的`Value`为实际存储的值；当`ok == false`时，说明`Key`不存在，此时返回的`Value`为该类型的零值。

如果需要明确区分零值`Value`与`Key`不存在的情况，或者`Value`的类型为`interface{}`，就必须接收两个返回值，并通过判断`ok`来确认`Key`是否存在。只有在可以明确确定该`Key`一定存在的前提下，才适合只接收一个返回值。
