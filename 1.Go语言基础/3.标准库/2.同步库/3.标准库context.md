...
`WithValue`的使用方式相对简单，一处写入，另一处读取即可。示例代码如下：

```go
func main() {
	ctx := context.Background()
	process(ctx)
    ctx = context.WithValue(ctx, "trace_id", "7415407145912410112")
	process(ctx)
}
...
但需要注意的是，并非所有数据都适合存储在`context`中。对于可以通过函数参数直接传递的值，应优先采用参数传递，`Go`语言官方明确建议避免将`context.Context`作为通用的数据传输容器，因为这并非`context`的设计初衷。

在`Go`的`context`设计中，`key`被刻意定义为`interface{}`类型，其核心目的在于避免跨包、跨模块场景下发生`key`冲突。若将`key`直接定义为字符串，例如`"trace_id"`，一旦其他包或模块恰好使用了相同的字符串作为`key`，就可能导致值被覆盖或被错误读取。这类问题通常具有高度隐蔽性，且在实际排查过程中极其困难。

`Go`官方在`context`包文档中给出的规范是：`key`应该使用一个不导出的自定义类型，并且通常是一个零值的`struct{}`或一个自定义类型的`const`。所以上面代码正确的写法应如下所示：
...
func main() {
	ctx := context.Background()
	process(ctx)
	ctx = context.WithValue(ctx, traceIdKey, "7415407145912410112")
	process(ctx)
}
