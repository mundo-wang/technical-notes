...
`sync.Cond`是`Go`语言中标准库`sync`包提供的一个条件变量（`Condition Variable`）实现。
...
`cond`对象创建完成后，这个互斥锁对象可以用`cond.L`来获取到。

`cond`对象有三个主要的方法，这里我们做一下讲解：

1. `Wait`方法

在调用`Wait`之前，通常需要先获取与`cond`关联的锁。调用`Wait`后，当前`Goroutine`会进入等待状态，执行流程在此处阻塞，同时释放该`cond`所关联的锁，直到有其他`Goroutine`（包括主`Goroutine`）调用`Signal`或`Broadcast`对其进行唤醒。当前`Goroutine`被唤醒后，会重新尝试获取该锁，只有在成功获取锁之后，`Wait`方法才会返回，程序才会继续向下执行。

2. `Signal`方法

`Signal`方法用于唤醒正在等待该条件的一个`Goroutine`。当存在多个等待中的`goroutine`时，`Signal`只会唤醒其中一个，但具体唤醒哪一个是不确定的。被唤醒的`Goroutine`会在返回前重新竞争并获取锁，随后继续执行后续逻辑。

3. `Broadcast`方法

`Broadcast`方法用于唤醒所有正在等待该条件的`Goroutine`。所有被唤醒的`Goroutine`都会进入就绪状态，并依次尝试获取锁；只有成功获取锁的`Goroutine`才能继续执行，其余的则会继续等待锁释放。

执行`Signal`与`Broadcast`方法前，也需要获取与`cond`关联的锁，在执行完成后释放该锁。
...
3. 接下来主函数唤醒所有`Goroutine`，`doSome`和`doOther`都被唤醒，它们再次尝试获取锁，并继续向下执行。
...
其中后两句打印的顺序是不固定的，因为同时唤醒两个`Goroutine`后，不知道哪个会先获取到锁。

`sync.Cond`本身并不保存任何业务条件，它只做三件事：

- 与一个`Mutex`绑定，用于保护共享状态。
- 提供`Wait`方法，让`Goroutine`在条件不满足时进入等待队列。
- 提供`Signal`与`Broadcast`方法，在条件可能发生变化时通知等待者重新检查条件。
