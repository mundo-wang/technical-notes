...
2. `fmt.Errorf()`：用于创建带有格式化消息的错误，或者用于包装错误，功能类似于`fmt.Sprintf`。
...
在多层调用链中，如果每一层都给可能出现的`err`记录日志，会导致同一次错误产生多条重复日志，不仅增加日志噪音，也容易造成错误数量统计失真，影响问题定位。更推荐的方式是明确日志写入边界，中间层只负责补充错误上下文并向上返回，最上层统一记录日志。错误本身通过包装携带完整调用链信息，最终日志只需要输出一次即可获得完整的错误传播路径。

错误包装通常使用`fmt.Errorf`配合`%w`占位符：

```go
return fmt.Errorf("getUserByID: %w", err)
```

如果使用`%v`格式化错误，仅会将错误转换为字符串，不会保留原始错误之间的关联关系，因此，经过`%v`处理后的错误无法继续被`errors.Is`和`errors.As`识别。而`%w`会将原始错误嵌入新的错误对象中，保留完整的错误链，使上层代码能够沿错误链继续匹配底层错误。需要注意的是，`%w`仅支持在`fmt.Errorf`中使用。
...
在包装错误时，应将当前层作用域下能够提供的上下文信息附加到错误中，确保错误在向上传递过程中持续保留关键调用信息，最终便于统一日志记录和问题定位。格式如下所示：

```go
return fmt.Errorf("getUserByID[user_id=%d]: %w", userId, err)
```

如果有多个变量，应仍然保持`key=value`形式，用逗号或空格分隔即可，例如：

```go
return fmt.Errorf("UserService.GetUser[user_id=%d tenant_id=%d]: %w", userId, tenantId, err)
```
