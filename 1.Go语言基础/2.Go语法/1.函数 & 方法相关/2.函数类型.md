...
`Go`语言中还有一个概念叫做闭包，闭包指的是一个函数内部引用了其外部函数作用域中的变量，并在外部函数返回后依然能够访问和修改这些变量的现象。其代码示例如下：
...
在上面的代码中，内部函数`func() int`就是一个闭包，因为它引用了外部函数`MakeIncrementer`中的局部变量`sum`。
