...
在定义类型约束时，我们可以无需手动编写，`constraints`库提供了一组可复用的通用约束，专为泛型设计。该库目前处于实验阶段，未来可能会有所调整，甚至被纳入标准库。我们通过`go get`命令将其引入：
...
func (pair *Pair[K, V]) PairToMap() map[K]V {
	result := make(map[K]V)
...
```go
type Processer[T constraints.Ordered] interface {
	Process(data T) T
	Save(data T) error
}
...
`Processer[T]`实例化后即成为具体接口类型。当接口类型指定为`Processer[string]`时，结构体`StringProcessor`相当于实现了该接口，其对象可赋值给该接口类型。但在接口中使用泛型违背了接口设计初衷，增加了代码复杂度，因此不建议采用此方式。
