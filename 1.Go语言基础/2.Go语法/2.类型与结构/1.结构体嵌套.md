...
匿名嵌套是指将一个结构体嵌套在另一个结构体中，但不为嵌套的结构体指定字段名。匿名嵌套结构体又分为直接嵌套和指针嵌套，以下是直接嵌套结构体的示例代码：
...
直接嵌套结构体嵌入父结构体`Address`为值类型，`Person`子结构体包含整个 `Address`父结构体的副本。而指针嵌入结构体嵌入父结构体`Address`的指针，`Person`子结构体持有`Address`父结构体的指针，避免了复制整个`Address`结构体。

除了在内存分配上的不同，还有就是指针嵌入可能会有空指针问题，例如下面这种情况：
...
由于`Address`对象未初始化，如果使用指针嵌套，代码会报错：`panic: runtime error: invalid memory address or nil pointer dereference`。而如果使用直接嵌套，在创建`Person`对象时，即使不显式创建`Address`对象，它仍会被默认创建，并赋予所有字段为其零值。因此，`Address`对象的`City`字段将是空字符串，调用时不会引发空指针错误。

> **注意：**在一般情况下，做结构体匿名嵌套（组合）的时候，应该使用直接嵌套结构体的方式。

我们创建子结构体`Person`的对象时，对于父结构体的字段，不能在外部直接给其赋值，需要先初始化父结构体，再给里面的字段赋值。也就是说，下面这种赋值方式是错误的：
...
如果子结构体`Person`和父结构体`Address`有同名字段，例如这样：
