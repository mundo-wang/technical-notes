...
也就是说，`int`的大小并不是固定的，而是随平台变化的，对应的无符号类型`uint`的行为和`int`一样，也是平台相关的。
...
整型包括`int`、`int8`、`int16`、`int32`、`int64`、`uint`、`uint8`、`uint16`、`uint32`、`uint64`。
...
> **注意：**在将浮点型转换为整型时，会发生向下取整，小数部分会被直接截断，因此在精度敏感的场景中需要格外谨慎使用。
...
`ok`是一个布尔类型值，用于表示类型断言是否成功。当断言失败时，`value`的值将是`Type`类型的零值，如果`Type`是引用类型，则`value`为`nil`。`Type`不仅可以是基本数据类型，也可以是自定义结构体类型。

> 如果被断言的变量`x`为`nil`，不会触发空指针异常，而是被视为断言失败，此时`ok`返回`false`，`value`的值为`nil`。
...
在对自定义结构体进行类型断言时，需要注意一种特殊场景，接口本身不为`nil`，但其内部保存的具体值却是`nil`：

```go
var d *Dog = nil
var a Animal = d // 此时对象a内部是 (type=*Dog, value=nil)

dog, ok := a.(*Dog)
```

例如接口持有的类型信息为`*Dog`，但其动态值为`nil`。此时进行类型断言，`ok`的结果为`true`，且`dog`的值为`nil`。虽然断言成功，但若直接调用`dog`对象的字段或方法，会触发`panic`。
...
类型断言可以省略第二个返回值`ok`，这不等同于使用`_`忽略该变量，忽略是指用`_`代替`ok`变量，而省略是直接不写该变量。当省略`ok`时，若断言失败，将直接触发`panic`，如下所示：
...
在开发中，我们不要使用上面省略`ok`参数的写法，而是要对断言是否成功进行判断，这样可以增强程序的健壮性。
...
当需要对一个接口值，例如`interface{}`或某个具体接口类型，在运行期可能承载的多种底层类型进行分支判断时，采用`switch case`形式的类型断言会更加直观，也更利于后续维护。在`Go`中，这种写法被称为类型选择，即`type switch`。
...
这里的`type`不再表示某个具体类型，而是`Go`语言内置的关键字。`.(type)`这种语法形式只能出现在`switch`语句中，不能单独使用。在每个`case`中，只能指定一个具体类型；当匹配到对应的`case`时，变量`v`会被自动推断为该`case`所对应的具体类型，随后执行相应的逻辑，并在执行完成后退出`switch`语句。`default`分支是可选的，通常用于兜底处理那些未被显式覆盖的类型情况。
...
func printValue(x interface{}) {
	...
	case int64:
		fmt.Println("int64类型，值为:", v)
	case float64:
		fmt.Println("float64类型，值为:", v)
	case string:
		fmt.Println("string类型，值为:", v)
	default:
		fmt.Println("未知类型:", reflect.TypeOf(x))
	}
}
```
...
func (w *WechatPay) Pay(amount int64) {
	fmt.Println("使用微信支付，金额:", amount)
}

type AliPay struct{}

func (a *AliPay) Pay(amount int64) {
	fmt.Println("使用支付宝支付，金额:", amount)
}
```

我们可以用接口类型作为函数入参，并断言其具体实现类型：

```go
func HandlePayment(p Payment) {
	switch v := p.(type) {
	case *WechatPay:
		fmt.Println("当前是微信支付实现")
		v.Pay(100)
	case *AliPay:
		fmt.Println("当前是支付宝支付实现")
		v.Pay(100)
	default:
		fmt.Println("未知支付方式")
	}
...
需要注意的是，类型断言不会进行任何形式的兼容判断，值类型与指针类型会被严格区分，并被视为两种完全不同的类型。
