...
我们还知道有种特殊的`channel`叫单向`channel`，分为只读管道和只写管道，但它们一般只被用在函数的形参，使用`make`创建一个单向`channel`没有任何意义。单向`channel`在易混语法章节中有详细讲解。
...
在下面的代码中，有两个任务任务同时就绪时，`select`会以伪随机的方式选择其中一个分支执行：

```go
highChan := make(chan string, 1)
lowChan := make(chan string, 1)
highChan <- "highMessage"
lowChan <- "lowMessage"
...
这是为了实现一个关键的并发设计目标，即避免`channel`饥饿问题。如果语言层面规定必须先处理`highChan`再处理`lowChan`，那么`highChan`将始终占据优先位置，导致`lowChan`可能长期甚至永久无法获得执行机会。

但在实际业务场景中，我们往往需要引入优先级机制：一旦存在高优先级任务，就必须立即处理；只有在没有高优先级任务时，才允许处理低优先级任务。这种情况就需要嵌套`select`来实现优先级，代码如下：
...
这段代码的语义发生了变化：首先立即检查高优`channel`是否有数据；如果有，则直接执行并结束，低优不会参与竞争；如果没有，则进入`default`分支，执行第二层`select`，此时才允许低优参与竞争。

第二层仍保留`highPriority`，是因为第一层检查与第二层执行之间存在极短时间窗口。在此期间若高优任务到达，而第二层未继续监听，就可能被低优抢占，破坏优先级。因此必须再次监听高优，保证调度正确性。
...
下面展示一个利用`channel`实现进程间通信，并让三个协程按顺序输出`one`、`two`、`three`，每个协程输出`100`次的代码：
...
如果需要进一步进阶，例如让三个协程按固定顺序轮流执行，协同完成从`1`到`100`的顺序输出，可以引入一个共享变量`num`，用于表示当前需要输出的数字。每个协程在接收到自己的`channel`信号后，执行以下流程：

- 判断`num`是否已大于`100`，若是，则直接结束当前协程的执行，并广播通知其他协程结束执行，否则进入下一步。
- 打印出当前的`num`，随后对`num`进行自增操作，并将执行权通过`channel`传递给下一个协程。

在实际实现过程中，需要重点关注对共享变量`num`的并发安全控制，并在协程退出阶段妥善处理`channel`的关闭与信号传递逻辑，否则极易引发数据竞争或死锁等问题。此处的代码不再展开说明。
