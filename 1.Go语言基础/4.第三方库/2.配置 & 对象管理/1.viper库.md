`viper`库是一个读取配置文件信息的库，它可以处理`json`、`yaml`、`properties`等文件。首先安装`viper`库：

```bash
go get github.com/spf13/viper
```

在初始化`viper`时，我们需要指定要读取的配置文件，并将其解析到`viper`的实例中，以便后续使用。

例如，我们希望读取`conf`目录下的`config.yml`配置文件，其目录结构如下：
...
> 这里的「相对路径」不是相对这行代码所在文件的路径，而是相对于距离该文件最近的`GoModules`管理的目录的相对路径。也就是说，`Go`编译器会从最近的包含`go.mod`文件的目录开始解析这个路径。

首先指定配置文件的目录路径，同样为相对项目根目录的路径。接着指定配置文件名：
...
接下来我们使用`ReadInConfig()`函数将配置文件内容解析到`viper`实例中：
...
该方法适用于在编译阶段固定运行环境，特别适合于`CI/CD`流程中，通过在不同阶段（如测试阶段、上线阶段）明确绑定特定环境，从而降低配置错误的风险，确保部署过程的稳定性和一致性。
...
我们可以使用`embed`包将配置文件打包进二进制文件里。编辑`init.go`文件，定义一个全局变量，并通过`go:embed`注释，将`config`包中的所有配置文件嵌入到二进制文件：

```go
//go:embed config/*.yml
var configsFS embed.FS // 把所有配置文件打包进embed.FS对象configsFS
```

> 使用`go:embed`时，匹配路径是相对于当前`Go`源文件所在目录计算的，而不是相对于项目根目录。此外，`go:embed`不允许路径中包含`..`来访问上级目录，路径必须指向当前源文件所在目录或其子目录中的文件。这也是上面调整目录结构的原因。

如果如果嵌入的是单个文本文件，可以直接映射为`string`或`[]byte`类型，如下所示：
...
这种情况下，需要我们使用`import _ "embed"`把这个包匿名引入该文件，否则会发生编译错误。

接下来，我们修改`InitConfig`函数，通过命令行参数的方式，动态获取需要读取的配置文件名称：
...
`ReadFile`方法返回的`data`是一个`[]byte`对象，我们初始化`viper`对象，并从字节流中读取配置：
...
通过这种方式，我们可以正常打包用于`Linux`环境的二进制文件，而无需手动同步`yml`配置文件。只需在`./`执行程序时通过命令行参数传入环境标识，例如：

```sh
./imchat --env=prod
```

即可实现多环境配置文件的自动切换。如果是`Docker`容器，可以在容器启动时通过`-e`传递环境变量：

```sh
docker run -e env=prod <image-name>
```
