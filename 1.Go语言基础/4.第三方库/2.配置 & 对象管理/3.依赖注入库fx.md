...
`fx.New`返回的`*fx.App`对象是一个可控的生命周期对象，调用其`Run()`方法将触发完整的应用启动流程，内部主要分为以下几个阶段：首先调用`Start`方法，触发所有通过`fx.Invoke`注册的函数并启动生命周期管理；随后进入阻塞状态，等待退出信号；收到退出信号后，调用`Stop`方法执行应用的优雅关闭流程。
...
与`Run()`不同，`Start`方法本身不会阻塞等待退出信号，其内部是同步执行的，会依次驱动依赖构建、执行`Invoke`函数并调用所有`OnStart`钩子，全部完成后即返回。因此在使用`Start`替代`Run()`时，需要在调用方手动监听退出信号并调用`Stop`，否则`OnStop`钩子将不会被触发，导致资源无法正确释放。此外，单个`OnStop`的错误不会中断后续钩子的执行，若存在多个错误，`Stop`方法最终返回的是这些错误的聚合值。使用`Start`与`Stop`方法的代码如下所示：
