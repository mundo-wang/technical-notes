...
fx.Invoke(
    fx.Annotate(
        func(mysqlDB *gorm.DB, object SomeObject) {
            // 具体业务逻辑
        },
        fx.ParamTags(`name:"mysql"`, ``),
    ),
...
`fx.Provide`、`fx.Invoke`、`OnStart`与`OnStop`都可能返回`error`。若产生`error`，`fx`内部会调用`os.Exit`，进程以非零退出码直接终止，错误信息通过`fx`的`Logger`输出。`Run`方法内部的源码如下所示：
...
如果需要集中获取错误并进行自定义处理，应使用`app.Start(ctx)`与`app.Stop(ctx)`代替`app.Run()`，因为这两个方法会接收返回`error`，可在调用方统一接收并处理。`Start`与`Stop`的方法签名如下所示：
...
与`Run()`不同，`Start`方法本身不会阻塞等待退出信号，其内部是同步执行的，会依次驱动依赖构建、执行`Invoke`函数并调用所有`OnStart`钩子，全部完成后即返回。因此需要在调用方手动监听退出信号并调用`Stop`，否则`OnStop`钩子将不会被触发，导致资源无法正确释放。此外，单个`OnStop`的错误不会中断后续钩子的执行，若存在多个错误，`Stop`方法最终返回的是这些错误的聚合值。
