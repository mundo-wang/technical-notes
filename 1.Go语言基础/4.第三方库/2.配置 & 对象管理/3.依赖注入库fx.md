...
`fx.New`返回的`*fx.App`对象是一个可控的生命周期对象，调用其`Run()`方法会触发完整的应用启动流程，其内部主要包含以下步骤：首先调用`Start`方法，触发所有通过`fx.Invoke`注册的函数，并启动生命周期管理；随后进入阻塞状态等待退出信号；在接收到退出信号后，调用`Stop`方法执行应用的优雅关闭流程。
