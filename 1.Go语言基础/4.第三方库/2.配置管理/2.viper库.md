...
上文中，我们直接使用包名`viper`调用函数，这是使用了`viper`包提供的全局变量来操作配置文件。其源码如下：
...
```go
myViper := viper.New()
myViper.SetConfigFile("conf/config.yml")
...
fmt.Println(utils.Config.Configurations.Settings.APIKey) // my-api-key
```
