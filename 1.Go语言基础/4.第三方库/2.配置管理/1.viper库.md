...
上述代码可以放置在`utils`目录下的`config.go`文件中，封装在`InitConfig`函数内，并在主函数中显式调用它：
...
我们调整了配置文件相关结构：将`conf`目录下的`config.go`重命名为`struct.go`，并将`Config`结构体改名为`YamlConfig`，将原位于`utils`目录的`config.go`移动至`conf`目录，并改名为`init.go`，同时，将原存放在`conf`目录下的所有`.yml`配置文件迁移至`conf/config`目录。调整后的目录结构如下：
