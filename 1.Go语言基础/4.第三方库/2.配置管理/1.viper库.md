...
我们对配置文件相关结构进行了调整：将`conf`目录下的`config.go`文件重命名为`struct.go`，并将原有的`Config`结构体更名为`YamlConfig`；将原本位于`utils`目录的`config.go`文件移动至`conf`目录并重命名为`init.go`；同时，将原存放在`conf`目录下的所有`.yml`配置文件统一迁移至`conf/config`目录。调整后的目录结构如下：
