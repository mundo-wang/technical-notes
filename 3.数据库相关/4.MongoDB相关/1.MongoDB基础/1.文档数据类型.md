...
`ObjectId`类型，是`MongoDB`中每个文档的默认唯一标识符，长度为`12`字节。它由时间戳、机器标识、进程`ID`和计数器组成，确保了其全局唯一性和有序性：

```json
{ "_id": { "$oid": "64b88a1e4a20f736cb6a82ab" } }
```

在`MongoDB`中，字段名`_id`是每个文档的默认唯一标识符，通常由`MongoDB`自动生成。默认情况下，`MongoDB`会为每个文档自动创建一个`ObjectId`类型的`_id`字段。如果用户自定义了`_id`字段，则该字段的值必须在集合内保持唯一，否则会触发错误。

`Date`类型，用于表示日期和时间，精确到毫秒：

```json
{ "createdAt": { "$date": "2025-01-06T03:42:50.303Z" } }
```

`Binary`类型，用于存储二进制数据，如文件、图像：
...
除了`ObjectId`、`Date`和`Binary`这三种类型外，`BSON`还支持`DBRef`、`Timestamp`、`MinKey`、`MaxKey`、`Undefined`、`Regular Expression`等数据类型，这些类型在这里暂时不做演示。
