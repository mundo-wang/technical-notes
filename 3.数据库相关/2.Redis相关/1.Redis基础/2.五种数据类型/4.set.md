...
`set`的结构为：单个`key`对应多个无序`member`，其中`member`不能重复。
...
> `set`类型不支持值为空`set`。当集合中所有元素被移除后，对应的键会被`Redis`自动删除。
...
- `count > 0`：返回`count`个唯一元素，不会有重复元素。
...
这里的`count`只能是非负整数，不存在`count < 0`的语义。当`count`大于集合的基数时，会直接移除整个集合。
