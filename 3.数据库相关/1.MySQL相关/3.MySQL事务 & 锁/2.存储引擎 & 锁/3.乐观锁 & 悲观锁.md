...
悲观锁假设数据冲突频繁，因此在操作数据之前先加锁，防止其他事务修改同一数据。

在`MySQL`中，悲观锁通常通过数据库原生的行级锁实现。普通`SELECT`是快照读，而以下两种操作属于当前读：

- `SELECT ... FOR UPDATE`：对选中的行加排它锁，允许其他事务快照读，但阻止其他事务当前读、修改或删除这些行。

- `SELECT ... LOCK IN SHARE MODE`：对选中的行加共享锁，允许其他事务快照读或当前读，但阻止修改或删除这些行。
