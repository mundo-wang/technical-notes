...
除了分组函数列以外，其他列必须全部出现在`GROUP BY`之后，否则会报错：

```
must appear in the GROUP BY clause or be used in an aggregate function
```

也就是说，`SELECT`子句中出现的列，要么必须出现在`GROUP BY`子句中，要么必须作为分组函数的参数使用。
...
有一个特殊情况：在`PostgreSQL`中，可以在`GROUP BY`后面跟`SELECT`设置的别名，但是`HAVING`后面不能跟。在`MySQL`中，`GROUP BY`和`HAVING`后面都可以跟`SELECT`设置的别名，这是数据库对`SQL`语句语法做出的优化。

例如，下面这条语句在`MySQL`中合法，但是在`PostgreSQL`中不合法（需要把`HAVING`后面的`COUNT`改为`COUNT(*)`）：
