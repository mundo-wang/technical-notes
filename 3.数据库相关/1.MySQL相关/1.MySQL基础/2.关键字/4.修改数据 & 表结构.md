...
在`MySQL`中，当你在某个字段上添加唯一约束（`UNIQUE`）时，`MySQL`会自动在该字段上创建一个唯一索引（`UNIQUE INDEX`），如果没有显式指定索引名，索引名默认与字段名相同。这个索引不仅保证字段值唯一，还能提高基于该字段的查询效率。

如果想显式指定索引名，也可以使用如下命令创建唯一约束：
...
建表与修改表的`DDL`需要严格遵守以下规范：

1. 主键：字段名`id`，类型`BIGINT NOT NULL AUTO_INCREMENT`，在表末尾定义`PRIMARY KEY (id)`。
2. 时间字段统一用`DATETIME`，其中`created_at`为`NOT NULL DEFAULT CURRENT_TIMESTAMP`，`updated_at`为`NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP`，`deleted_at`为`DEFAULT NULL`。
3. `VARCHAR`字段：业务上不可为空则`NOT NULL`不设默认值；无需区分`NULL`与空字符串则`NOT NULL DEFAULT ''`。
4. 数值字段：不可为空则`NOT NULL`不设默认值；无需区分`NULL`与`0`则`NOT NULL DEFAULT 0`；`0`有业务含义则`DEFAULT NULL`。
5. 所有表名、字段名用反引号包裹，表与字段均须有`COMMENT`。
6. 除主键外不创建额外索引（有特别说明除外）；禁用外键，表关联通过在子表存主表`id`实现。
