...
- `varchar`：可变长度的字符串类型，定义时必须指定最大长度，格式为`varchar(M)`，其中`M`表示该字段最多可存储的字符数。其存储空间根据实际字符数动态分配，例如，`varchar(100)`最多可存储`100`个字符，若实际存储`10`个字符，则仅占用`10`个字符的存储空间。

- `char`：固定长度的字符串类型，默认最大长度为`1`，通常建议指定最大长度，格式为`char(M)`，其中`M`表示该字段存储的固定字符数，如果存储的字符少于`M`，则会使用空格填充至指定长度。例如，`char(20)`表示存储长度固定为`20`的字符串，不足部分会用空格补齐。

- `text`：用于存储大文本数据，支持可变长度字符串，能存储比`varchar`更大的文本内容。`text`类型的数据通常存储在专用的数据页中，适用于存储大量文本信息。

使用场景：

- 对于短字符串或固定长度的字符串（如`UUID`），适合使用`char`，以提高存储和查询效率。
- 在大多数情况下，`varchar`是更优的选择，能够灵活存储变长字符串并节省空间。
- `text`适用于存储大量文本数据，但不适合用于创建索引或在`where`子句中进行高效查询。
