...
- `VARCHAR`：可变长度的字符串类型，定义时必须指定最大长度，格式为`VARCHAR(M)`，其中`M`表示该字段最多可存储的字符数。其存储空间根据实际字符数动态分配，例如，定义`VARCHAR(100)`时，表示该字段最多可存储`100`个字符，若实际存储`10`个字符，则仅占用`10`个字符的存储空间。

- `CHAR`：固定长度的字符串类型，默认最大长度为`1`，建议明确指定长度，格式为`CHAR(M)`，其中`M`表示固定的存储字符数。如果存储的字符少于`M`，会使用空格填充至指定长度。例如，`CHAR(20)`表示该字段固定存储`20`个字符，不足部分用空格补齐。
...
`MySQL`中`NOW()`函数返回的当前时间是`DATETIME`类型。不用纠结选择`TIMESTAMP`还是`DATETIME`，全部选用`DATETIME`就好。

在数据库层面，可以设置时间字段的默认行为，插入记录时自动填充当前时间，更新记录时自动刷新更新时间字段：
...
对于`deleted_at`字段，应该设置为`DEFAULT NULL`，以表示记录未被删除的初始状态。
...
在`PostgreSQL`中有一种特殊的数据类型叫做数组。它通过在数据类型后加上`[]`来表示，如下所示：
...
需要注意，在`MySQL`中推荐在表名和字段名两侧使用反引号包裹，以避免与关键字冲突。而在`PostgreSQL`中，表名和字段名的标识符定界符是双引号，而不是`MySQL`中的反引号。如果在`PostgreSQL`中使用反引号，则会报语法错误。
