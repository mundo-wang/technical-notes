...
在`MySQL`中，使用`now()`函数获取的当前时间是`datetime`类型。如果对于`datetime`类型列设置了`DEFAULT NULL`，该列的默认值会是`0000-00-00 00:00:00`。然而，`MySQL`不接受`0000-00-00 00:00:00`作为有效的`DATETIME`类型值，因此会导致报错：

```sh
Error 1292 (22007): Incorrect datetime value: '0000-00-00' for column 'login_time' at row 1
```

因此，建议为该字段设置如下默认值，以避免因无效`DATETIME`值而导致的数据库报错：
