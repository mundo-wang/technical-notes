...
当字段中`NULL`或非`NULL`值占比较高，导致索引选择性较低时，走索引可能需要扫描大量索引记录，并产生大量回表操作。此时，索引扫描带来的随机`I/O`和`CPU`开销可能高于直接进行顺序全表扫描，因此优化器通常会放弃索引。

具体而言，在查询需要回表（非覆盖索引，如`SELECT *`）前提下：

- 当`NULL`值占比较高时，使用`IS NULL`查询通常不会走索引。
- 当`NULL`值占比较低时，使用`IS NOT NULL`查询通常不会走索引。

需要注意的是，这并不是固定规则，最终是否使用索引，需要通过`EXPLAIN`查看执行计划确认。
